        ...
        <nav style={{ display: "flex", alignItems: "center", gap: 2, flex: 1 }}>
          {NAV_ITEMS.map((item) => (
            <button
              ...
              onMouseEnter={(e) => {
                if (activeKey !== item.key) e.currentTarget.style.color = S.ink;
              }}
              onMouseLeave={(e) => {
                if (activeKey !== item.key)
                  e.currentTarget.style.color = S.ink3;
              }}
            >
              {item.label}
              {activeKey === item.key && (
...
export function MarketTickerStrip({ compact }: { compact?: boolean }) {
  const [quotes, setQuotes] = useState<IndexQuote[]>(
    INDICES.map((i) => ({ ...i, price: null, change: null })),
  );
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      if (typeof api.getQuote !== "function") return;
    ...
    >
      <div
        style={{
          ...
          borderRadius: compact ? S.radiusSm : S.radiusMd,
      ...
      >
        {quotes.map((q) => (
          ...
          >
            ...
            {q.price !== null ? (
              ...
              <span style={{ fontSize: compact ? 11 : 12, color: S.ink4 }}>
                —
              </span>
            )}
          </div>
        ...
        >
          ...
          <span style={{ fontSize: compact ? 10 : 11, color: S.ink4 }}>
            Closes 3:30 PM
          </span>
        </div>
      </div>
    ...
    background:
      variant === "primary"
        ...
        : variant === "ghost"
          ...
          : "transparent",
    color:
      variant === "primary" ? "white" : variant === "ghost" ? S.ink3 : S.ink2,
  };
  return (
    <button style={{ ...base, ...extStyle }} onClick={onClick}>
      {children}
    </button>
  );
}
...
export function PremiumCard({
  ...
  return (
    <div
      ...
      onMouseEnter={(e) => {
        if (onClick) e.currentTarget.style.boxShadow = S.shadowFloating;
      }}
      onMouseLeave={(e) => {
        if (onClick) e.currentTarget.style.boxShadow = S.shadowCard;
      }}
    >
      {children}
    </div>
...
export function FactorBar({
  ...
  const pct =
    score !== null
      ? (Math.max(0, Math.min(maxScore, score)) / maxScore) * 100
      : 0;
  return (
...
export function MetricCard({
  ...
  change?: { value: number; isUp: boolean } | null;
      ...
      {change && (
        <span
          style={{
            fontSize: 12,
            fontWeight: 600,
            color: change.isUp ? S.positive : S.negative,
          }}
        >
          {change.isUp ? "+" : ""}
...
export function MiniSparkline({
  ...
  const points = data
    ...
    .join(" ");
  const c =
    color || (data[data.length - 1] >= data[0] ? S.positive : S.negative);
  return (
...
export function PerformanceChart({
  ...
  if (data.length < 2)
    return (
      <div style={{ height, background: S.bgSoft, borderRadius: S.radiusSm }} />
    );
  const min = Math.min(...data);
  const max = Math.max(...data);
  const range = max - min || 1;
  ...
  const points = data
    .map(
      (v, i) =>
        `${(i / (data.length - 1)) * w},${height - ((v - min) / range) * (height - 20) - 10}`,
    )
    .join(" ");
      ...
      {tabs.map((t) => (
        <button
          ...
          style={{
            ...
            color: activeTab === t ? S.ink : S.ink3,
            border: "none",
            background: "none",
            cursor: "pointer",
            borderBottom:
              activeTab === t ? `2px solid ${S.ink}` : "2px solid transparent",
            transition: "color 0.15s",
          }}
        >
          {t}
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
      ...
      <div>
        ...
        >
          <span style={{ fontSize: 12, fontWeight: 600, color: S.ink2 }}>
            {symbol}
          </span>
          <span
            style={{
          ...
          {sector && (
            <span style={{ fontSize: 11, color: S.ink3 }}>{sector}</span>
          )}
        </div>
      </div>
    </div>
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div
          style={{ fontSize: 12, color: S.ink3, marginTop: 2, lineHeight: 1.5 }}
        >
          {body}
        </div>
      </div>
...
/* ─────────────────────────────────────────────
   ScannerFilterRail
   ───────────────────────────────────────────── */
export function ScannerFilterRail({
  filters,
  onChange,
  onRun,
  onSave,
  ...
  return (
    ...
    >
      <div>
        ...
        >
          <h2
            style={{ fontSize: 15, fontWeight: 700, color: S.ink, margin: 0 }}
          >
            AI Stock Scanner
          </h2>
          <span
        ...
        <p style={{ fontSize: 11, color: S.ink3, margin: 0, lineHeight: 1.5 }}>
          Find high-quality, high-conviction stocks using AI and factor
          intelligence.
        </p>
      </div>
          ...
          {rows.map((row, i) => (
            <tr
              ...
              style={{
                ...
                borderBottom:
                  i < rows.length - 1 ? `1px solid ${S.borderSoft}` : "none",
              }}
              onMouseEnter={(e) =>
            ...
            >
              <td style={{ padding: "12px 14px", fontSize: 11, color: S.ink3 }}>
                {row.rank}
              </td>
              <td style={{ padding: "12px 14px" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  ...
                  <div>
                    <div
                      style={{ fontSize: 12, fontWeight: 600, color: S.ink }}
                    >
                      {row.symbol}
                    </div>
                    <div style={{ fontSize: 10, color: S.ink3 }}>
                      {row.name}
                    </div>
                  </div>
                </div>
                ...
                >
                  <span
                    style={{
                      ...
                      fontWeight: 700,
                      color:
                        row.score !== null ? scoreColor(row.score) : S.ink3,
                    }}
                  >
                    ...
                    return (
                      <span
                        ...
                        style={{
                          ...
                          borderRadius: 3,
                          background:
                            f.value !== null && f.value >= 70
                              ? S.positiveSoft
                              : f.value !== null && f.value < 40
                                ? "transparent"
                                : S.bgSoft,
                          color: c,
...
export function RightInsightRail({
  ...
  return (
    <div
      style={{ width: 240, display: "flex", flexDirection: "column", gap: 16 }}
    >
      <PremiumCard>
        <div
          style={{
        ...
        <p style={{ fontSize: 10, color: S.ink3, margin: "0 0 12px 0" }}>
          Why these stocks rank high today
        </p>
        {insights.map((ins, i) => (
          <InsightCard
            key={i}
      ...
      <PremiumCard>
        <div style={{ textAlign: "center", padding: "8px 0" }}>
          ...
          <p style={{ fontSize: 11, color: S.ink3, margin: "0 0 12px 0" }}>
            Save this scan configuration
          </p>
          {onSave && (
            <button
              onClick={onSave}
...
export function EmptyProductState({
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: 12, color: S.ink3, margin: 0, maxWidth: 320 }}>
        {body}
      </p>
    </div>
  );
...
export function HealthometerRing({
  score,
  classification,
  size = 72,
}: {
...
export function FactorBreakdownBars({
  factors,
}: {
  ...
  const ordered = [
    ...core,
    ...factors.filter((f) => !core.includes(f.group)).map((f) => f.group),
  ];
...
/* ─────────────────────────────────────────────
   ...
   ───────────────────────────────────────────── */
export function InvestmentReviewSheet({
  open,
  onClose,
  symbol,
  companyName,
    ...
    >
      ...
      <div
        style={{
          ...
          maxWidth: 480,
          background: S.surface,
          borderRadius: `${S.radiusLg} ${S.radiusLg} 0 0`,
          padding: "28px 24px",
          maxHeight: "85vh",
          overflow: "auto",
      ...
      >
        ...
        >
          <div>
            ...
            <span style={{ fontSize: 12, color: S.ink3, marginLeft: 8 }}>
              {symbol} · {companyName}
            </span>
          </div>
          <button
            onClick={onClose}
            aria-label="Close investment review"
            style={{
              border: "none",
              background: "none",
              cursor: "pointer",
        ...
        >
          ...
          >
            {thesis ||
              "Review the company thesis, research dimensions, and available financial context before deciding your next step."}
          </p>
        </div>
        ...
        >
          ...
          >
            {(risks.length
              ? risks
              : [
                  "No company-specific risk summary is ready yet. Review the full research page before continuing.",
                ]
            ).map((risk) => (
              <div
                key={risk}
              ...
              >
                <span style={{ fontSize: 10, color: S.negative, marginTop: 2 }}>
                  ●
                </span>
                <span style={{ fontSize: 12, color: S.ink2 }}>{risk}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        >
          ...
          >
            {[
              "Review the full thesis on the company page",
              "Compare with peer companies in the sector",
              "Consider your portfolio allocation and risk appetite",
        ...
        >
          Broker handoff is being prepared. Final order placement happens with
          your broker. StockStory does not handle your money or execute trades.
        </div>

        <div style={{ display: "flex", gap: 8 }}>
          <button
            onClick={() => productNavigate("track")}
            style={{
              flex: 1,
              padding: "12px",
              fontSize: 13,
...
/* ─────────────────────────────────────────────
   BrokerHandoffSheet
   ───────────────────────────────────────────── */
export function BrokerHandoffSheet({
  open,
  onClose,
  symbol,
}: {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          Broker handoff is being prepared. StockStory is a research platform —
          we help you decide, not execute.
        </p>
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <button
            onClick={() => {
              if (symbol) productNavigate("compare", symbol);
            }}
            style={{
              width: "100%",
              padding: "12px",
