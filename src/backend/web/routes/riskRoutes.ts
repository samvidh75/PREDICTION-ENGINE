...
import { complianceSelfCheckService } from '../../../services/risk/ComplianceSelfCheckService.js';
...
export async function registerRiskRoutes(app: FastifyInstance) {
  ...
  app.get('/api/risk/compliance', async (_request, _reply) => {
    return complianceSelfCheckService.runComplianceCheck();
  });

  app.get('/api/risk/compliance/audit-log', async (request, _reply) => {
    const query = request.query as Record<string, string>;
    return complianceSelfCheckService.queryAuditLog({
      userId: query.userId,
      action: query.action,
