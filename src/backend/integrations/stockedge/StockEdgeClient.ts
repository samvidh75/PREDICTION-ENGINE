...
export class StockEdgeClient {
  ...
  async getJson<T>(options: StockEdgeRequestOptions): Promise<StockEdgeFetchResult<T>> {
    ...
    const cacheKey = stockEdgeMemoryCache.key(options.layer, symbol, options.suffix ?? options.path ?? "default");
    ...
    return stockEdgeMemoryCache.coalesce(cacheKey, async () => {
      try {
        ...
        const session = await stockEdgeAuth.ensureSession();
        const value = await this.fetchJson<T>(options.path ?? `/api/stock/${symbol}`, session.cookieHeader);
        ...
        if (error instanceof StockEdgeIntegrationError && error.code === STOCKEDGE_CODES.sessionExpired) {
  ...
  private async fetchJson<T>(path: string, cookieHeader: string): Promise<T> {
    ...
    try {
      ...
      if (response.status === 401) {
        stockEdgeSessionStore.clearSession();
        throw new StockEdgeIntegrationError(STOCKEDGE_CODES.sessionExpired, "StockEdge session expired", false);
      }
      if (!response.ok) {
