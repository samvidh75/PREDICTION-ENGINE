...
describe("compareCompanies", () => {
  it("compares two companies and identifies the stronger case", () => {
    const r = compareCompanies([
      { symbol: "BDO", companyName: "BDO Unibank Inc", scores: { quality: 82, valuation: 48, growth: 65, risk: 70, momentum: 60, stability: 78 } },
      { symbol: "ICT", companyName: "ICT", scores: { quality: 75, valuation: 55, growth: 60, risk: 65, momentum: 55, stability: 70 } },
    ]);
    expect(r.companies).toHaveLength(2);
    expect(r.recommendation).toContain("BDO Unibank Inc");
  ...
  it("handles single company input", () => {
    const r = compareCompanies([{ symbol: "BDO", companyName: "BDO", scores: { quality: 80, valuation: 50, growth: 60, risk: 65, momentum: 55, stability: 70 } }]);
    expect(r.missingDataCaveat).toContain("at least two");
  });

  it("no forbidden labels in compare output", () => {
    const r = compareCompanies([
      { symbol: "BDO", companyName: "BDO", scores: { quality: 82, valuation: 48, growth: 65, risk: 70, momentum: 60, stability: 78 } },
      { symbol: "ICT", companyName: "ICT", scores: { quality: 75, valuation: 55, growth: 60, risk: 65, momentum: 55, stability: 70 } },
    ]);
