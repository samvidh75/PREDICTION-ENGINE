...
describe("normalizeQuote", () => {
  it("normalizes a valid quote", () => {
    const result = normalizeQuote({ symbol: "SM", lastPrice: 2500, change: 25, changePercent: 1.01 });
    expect(result.error).toBeNull();
    expect(result.data?.symbol).toBe("SM");
  ...
  it("handles null prices gracefully", () => {
    const result = normalizeQuote({ symbol: "BDO", lastPrice: null });
