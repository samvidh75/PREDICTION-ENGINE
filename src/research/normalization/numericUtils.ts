export function safeFinite(value: unknown): number | null {
  ...
  if (typeof value === "string") {
    // Strip the "₱" currency prefix, then thousands separators/percent/
    // whitespace — but never the decimal point itself, which a bare `.`
    // inside a character class would otherwise also match.
...
// Re-exported from canonical location in @/types
