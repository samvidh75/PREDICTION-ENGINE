...
/**
 * Score Market Cap on a 0-100 scale (Philippine-market oriented).
...
export function computeMarketCapScore(marketCap: number | null): number | null {
  ...
  const cr = marketCap / 1000000; // convert to PHP millions
  if (cr >= 2000000) return 90;   // Mega-cap (₱2T+)
  if (cr >= 500000) return 80;    // Large-cap (₱500B+)
  if (cr >= 200000) return 70;    // Mid-large (₱200B+)
  if (cr >= 50000) return 55;     // Mid-cap (₱50B+)
  if (cr >= 10000) return 40;     // Small-cap (₱10B+)
  if (cr >= 1000) return 25;      // Micro-cap (₱1B+)
  return 15;                       // Nano-cap
}
