...
import { normalizeTicker } from '../symbols/PSESymbolNormalizer';
import type { PSESymbolResolver } from '../symbols/PSESymbolResolver';
import type {
  EodIngestionBatch,
  EodIngestionResult,
  IngestionTaskRecord,
} from './PSEEodIngestionTypes';
...
export class PSEEodIngestionPipeline {
  constructor(
    private readonly resolver: PSESymbolResolver,
  ...
  async ingest(
    ...
    for (const raw of rawCandles) {
      ...
      // 3. Build PSEEodCandle
      const candle: PSEEodCandle = {
      ...
      const existing = await this.cache.get<PSEEodCandle>('eod_history', `${candle.symbol}:${candle.date}`);
      if (existing) {
