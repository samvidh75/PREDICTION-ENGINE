...
describe('Symbol master fixture integration', () => {
  ...
  it('has at least 15 PSE equities', () => {
    const equities = buildSymbolMasterFixture().filter(s => s.exchange === 'PSE' && s.segment === 'EQ');
    expect(equities.length).toBeGreaterThanOrEqual(15);
  });

  it('has at least 1 suspended or delisted symbol', () => {
    const inactive = buildSymbolMasterFixture().filter(s => s.listingStatus !== 'active');
