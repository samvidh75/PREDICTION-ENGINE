...
import { normalizeTicker, inferExchange, inferSegment, isPseCode } from '../symbols/PSESymbolNormalizer';
...
describe('inferSegment', () => {
  ...
  it('returns null for unknown', () => {
    expect(inferSegment('UNKNOWNCO')).toBeNull();
  });

  it('returns ET for known ETF tickers', () => {
    expect(inferSegment('FMETF')).toBe('ET');
  });
});

describe('isPseCode', () => {
  it('returns true for 6-digit codes', () => {
    expect(isPseCode('500325')).toBe(true);
  });

  it('returns true for 4-digit codes', () => {
    expect(isPseCode('0096')).toBe(true);
  });

  it('returns true for 8-digit codes', () => {
    expect(isPseCode('12345678')).toBe(true);
  });

  it('returns false for short strings', () => {
    expect(isPseCode('123')).toBe(false);
  });

  it('returns false for ticker-like strings', () => {
    expect(isPseCode('RELIANCE')).toBe(false);
    expect(isPseCode('TCS')).toBe(false);
  });

  it('returns false for empty string', () => {
    expect(isPseCode('')).toBe(false);
  });
});
