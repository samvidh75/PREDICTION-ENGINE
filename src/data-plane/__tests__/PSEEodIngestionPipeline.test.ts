...
import { PSEEodIngestionPipeline } from '../ingestion/PSEEodIngestionPipeline';
import type { EodDataCacheLike, RawCandleInput } from '../ingestion/PSEEodIngestionPipeline';
import type { PSESymbolResolver, SymbolResolutionResult } from '../symbols/PSESymbolResolver';
import type { PSESymbol } from '../symbols/PSESymbol';
import type { EodIngestionBatch } from '../ingestion/PSEEodIngestionTypes';
...
function makeResolver(results: Record<string, SymbolResolutionResult>): PSESymbolResolver {
  return {
    ...
    resolveByPseCode: vi.fn(async () => null),
...
describe('PhilippineEodIngestionPipeline', () => {
  let resolver: PSESymbolResolver;
  let cache: EodDataCacheLike;
  let pipeline: PSEEodIngestionPipeline;
  ...
  beforeEach(() => {
    ...
    cache = makeCache();
    pipeline = new PSEEodIngestionPipeline(resolver, cache);
  });
  ...
  it('returns batch descriptor in result', async () => {
    ...
    expect(result.batch.source).toBe('pse_daily_csv');
  });
  ...
  it('createTaskRecord produces valid task record', async () => {
    ...
    expect(record.source).toBe('pse_daily_csv');
