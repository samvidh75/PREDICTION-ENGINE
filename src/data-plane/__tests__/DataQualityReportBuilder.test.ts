...
function makeCandle(overrides?: Partial<PSEEodCandle>): PSEEodCandle {
  return {
    symbol: 'BDO', exchange: 'PSE', date: '2026-06-17',
...
describe('buildUniverseOverlapSummary', () => {
  it('reports sizes by universe', () => {
    ...
    universes.set('large_cap', symbols.filter(s => s.marketCapCategory === 'large'));
    universes.set('mid_cap', symbols.filter(s => s.marketCapCategory === 'mid'));

    const result = buildUniverseOverlapSummary(universes);
    expect(result.sizeByUniverse['large_cap']).toBeGreaterThan(0);
  ...
  it('finds common symbols', () => {
    ...
    const equities = symbols.filter(s => s.segment === 'EQ');
    const universes = new Map<string, PSESymbol[]>();
    universes.set('all', allActive);
    universes.set('equities', equities);
