...
import { StoreBackedSymbolResolver } from '../symbols/PSESymbolResolver';
import type { PSESymbol } from '../symbols/PSESymbol';
import type { PSESymbolMasterStoreLike } from '../symbols/PSESymbolResolver';
...
function makeSymbol(overrides: Partial<PSESymbol> & { canonicalSymbol: string }): PSESymbol {
  return {
    ...
    pseCode: null,
    pseSymbol: overrides.canonicalSymbol,
    faceValue: null,
    marketCapCr: null,
...
function createMockStore(): PSESymbolMasterStoreLike {
  const symbols: PSESymbol[] = [
    makeSymbol({ canonicalSymbol: 'RELIANCE', aliases: ['RELIANCE', 'RELIANCE.NS', 'RELIANCE-EQ', '500325'], isin: 'IN0020200124', pseCode: '500325' }),
    makeSymbol({ canonicalSymbol: 'TCS', aliases: ['TCS', 'TCS.NS', 'TCS-EQ', '532540'], isin: 'INE467B01029', pseCode: '532540' }),
    makeSymbol({ canonicalSymbol: 'HDFCBANK', aliases: ['HDFCBANK', 'HDFCBANK.NS', 'HDFCBANK-EQ', '500180'], isin: 'INE040A01034', pseCode: '500180' }),
    makeSymbol({ canonicalSymbol: 'INFY', aliases: ['INFY', 'INFY.NS', 'INFY-EQ', '500209'], isin: 'INE009A01021', pseCode: '500209' }),
  ];
  ...
  for (const s of symbols) {
    ...
    if (s.pseCode) byBse.set(s.pseCode, s);
  }
  ...
  return {
    ...
    findByPseCode: vi.fn((code: string) => Promise.resolve(byBse.get(code) ?? null)),
    listActive: vi.fn(() => Promise.resolve(symbols)),
  };
}

let mockStore: PSESymbolMasterStoreLike;
...
describe('StoreBackedSymbolResolver', () => {
  ...
  it('resolveByPseCode works', async () => {
    const s = await resolver.resolveByPseCode('500180');
