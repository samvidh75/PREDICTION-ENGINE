...
import type { PSESymbol, PSEExchange, PSEInstrumentSegment } from '../symbols/PSESymbol';
import type { PSEEodCandle } from '../eod/PSEEodCandle';
import { validateEodCandle } from '../eod/PSEEodCandle';
...
export interface SymbolHealthSummary {
  total: number;
  byExchange: Record<PSEExchange, number>;
  bySegment: Partial<Record<PSEInstrumentSegment, number>>;
  byCapCategory: Record<string, number>;
  withIsin: number;
  missingIsin: number;
  withPseCode: number;
  missingPseCode: number;
...
export function buildSymbolHealthSummary(symbols: PSESymbol[]): SymbolHealthSummary {
  ...
  for (const s of symbols) {
    ...
    if (s.pseCode) withPseCode++; else missingPseCode++;
  ...
  return {
    ...
    byExchange: byExchange as Record<PSEExchange, number>,
    bySegment: bySegment as Partial<Record<PSEInstrumentSegment, number>>,
    byCapCategory,
    withIsin,
    missingIsin,
    withPseCode,
...
export function buildEodQualitySummary(candles: PSEEodCandle[]): EodQualitySummary {
...
export function buildCalendarCoverageSummary(calendar: PSETradingCalendar): CalendarCoverageSummary {
  ...
  // Verify weekend consistency: every Saturday/Sunday in the covered years
  // must be skipped as a non-trading day. A holiday that legitimately falls
  // on a weekend (e.g. Black Saturday 2026-04-04) is NOT an error — the
  // check is that weekends are never treated as trading days.
  let weekendSkipsCorrect = true;
  if (holidayList.length > 0) {
    const years = new Set(holidayList.map((h) => h.slice(0, 4)));
    checkWeekends: for (const year of years) {
      const cursor = new Date(`${year}-01-01T00:00:00.000Z`);
      while (cursor.getUTCFullYear() === Number(year)) {
        const day = cursor.getUTCDay();
        if ((day === 0 || day === 6) && calendar.isTradingDay(cursor)) {
...
export function buildFullDataQualityReport(
  ...
  if (symbolHealth.missingPseCode > 0) {
    notes.push(`ℹ️  ${symbolHealth.missingPseCode} symbols missing PSE code (expected for PSE-primary).`);
  }
...
export function renderDataQualityReport(report: DataQualityReport): string {
  ...
  lines.push(`- **With PSE code:** ${report.symbolHealth.withPseCode}`);
