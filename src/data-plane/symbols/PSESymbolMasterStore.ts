...
import type { PSESymbol, PSEExchange, PSEInstrumentSegment, PSEListingStatus } from './PSESymbol';
...
function rowToSymbol(row: SymbolMasterRow): PSESymbol {
  return {
    ...
    exchange: row.exchange as PSEExchange,
    segment: row.segment as PSEInstrumentSegment,
    isin: row.isin,
    companyName: row.company_name,
    sector: row.sector,
    industry: row.industry,
    listingStatus: row.listing_status as PSEListingStatus,
    aliases: JSON.parse(row.aliases) as string[],
    pseCode: row.pse_code,
    pseSymbol: row.pse_symbol,
...
export class PSESymbolMasterStore {
  /**
  ...
  async upsert(symbol: PSESymbol): Promise<PSESymbol> {
    ...
    await runQuery(
      `INSERT INTO symbol_master (
        ...
        listing_status, aliases, pse_code, pse_symbol, face_value,
        ...
        pse_code = EXCLUDED.pse_code,
        pse_symbol = EXCLUDED.pse_symbol,
      ...
      [
        ...
        row.pse_code, row.pse_symbol, row.face_value, row.market_cap_cr,
  ...
  async findByAlias(alias: string): Promise<PSESymbol | null> {
    ...
    // Also check ISIN and pse_code
    const byIsin = await this.findByIsin(alias);
    if (byIsin) return byIsin;
    const byBse = await this.findByPseCode(alias);
   ...
   */
  async findByPseCode(code: string): Promise<PSESymbol | null> {
    const rows = await runQuery<SymbolMasterRow>(
      'SELECT * FROM symbol_master WHERE pse_code = $1',
      [code],
    );
    return rows.length > 0 ? rowToSymbol(rows[0]) : null;
  }
   ...
   */
  async count(listingStatus?: PSEListingStatus): Promise<number> {
...
export const symbolMasterStore = new PSESymbolMasterStore();
