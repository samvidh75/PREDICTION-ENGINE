...
import { normalizeTicker } from './PSESymbolNormalizer';
...
export interface SymbolResolutionResult {
  ...
  /** The lookup key that was actually matched (e.g. 'BDO'). */
  matchedKey: string;
}
 ...
 *
 * The canonical implementation uses `PSESymbolMasterStore`, but this
 * interface allows alternative backends (e.g. in-memory cache, test stub).
 */
export interface PSESymbolResolver {
  /**
   ...
   */
  resolveByPseCode(code: string): Promise<PSESymbol | null>;
...
/**
 * Resolver backed by a `PSESymbolMasterStore` instance.
 */
export class StoreBackedSymbolResolver implements PSESymbolResolver {
  constructor(private readonly store: PSESymbolMasterStoreLike) {}
  ...
  async resolve(raw: string): Promise<SymbolResolutionResult> {
    ...
    if (/^\d{4,8}$/.test(trimmed)) {
      const byBse = await this.store.findByPseCode(trimmed);
  ...
  async resolveByPseCode(code: string): Promise<PSESymbol | null> {
    return this.store.findByPseCode(code);
  }
 ...
 */
export interface PSESymbolMasterStoreLike {
  ...
  findByPseCode(code: string): Promise<PSESymbol | null>;
  listActive(): Promise<PSESymbol[]>;
}
