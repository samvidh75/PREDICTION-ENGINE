...
function translateSQL(sql: string): string {
  ...
  translated = translated
    ...
    // SQLite requires a non-constant column DEFAULT expression to be
    // wrapped in parens (`DEFAULT (datetime('now'))`), unlike Postgres —
    // without this, `... NOT NULL DEFAULT datetime('now')` is a DDL syntax
    // error.
  ...
  private async ensureTables(): Promise<void> {
    ...
    const tables = [
      ...
      `CREATE TABLE IF NOT EXISTS benchmark_observations (
        observed_date TEXT PRIMARY KEY, psei REAL, psei_top10 REAL, pse_all_shares REAL,
