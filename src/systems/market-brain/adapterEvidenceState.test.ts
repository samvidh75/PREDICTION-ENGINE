...
import { evaluatePSEEquity, type PSEEquityPacket } from './pseMarketBrain';

const readyEvidence: PSEEquityPacket['evidence'] = {
...
const strongPacket: PSEEquityPacket = {
  symbol: 'sample',
...
const UNSAFE_PUBLIC_COPY = /provider|\bAPI\b|backend|diagnostics|coverage|freshness|lineage|migration|backfill|source pending|source verified|quote unavailable|history unavailable|null adapter|ADAPTER_UNAVAILABLE|EMPTY_RESPOPSE|MALFORMED_RESPOPSE|\bBuy\b|\bSell\b|\bHold\b|Strong Buy|guaranteed|sure shot|multibagger/i;
...
describe('Market Brain adapter evidence wiring', () => {
  it('keeps empty adapter evidence from changing existing output', () => {
    const withoutAdapter = evaluatePSEEquity(strongPacket);
    const withAdapter = evaluatePSEEquity({
      ...strongPacket,
