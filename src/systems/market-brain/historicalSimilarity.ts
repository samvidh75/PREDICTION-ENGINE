...
const FEATURE_KEYS: (keyof HistoricalSimilarityFeatures)[] = [
  'priceMovePct',
  'volumeMultiple',
  'volatilityMultiple',
  'sectorMovePct',
...
function featureDistance(current: HistoricalSimilarityFeatures, candidate: HistoricalSimilarityFeatures): number | null {
  ...
  for (const key of FEATURE_KEYS) {
    ...
    total += Math.abs(currentValue - candidateValue) / denominator;
    count += 1;
  }
...
export function buildHistoricalSimilaritySummary(input: HistoricalSimilarityInput): HistoricalSimilaritySummary {
  ...
  const ranked = input.cases
    ...
    .slice(0, maxCases)
    .map((entry) => entry.historicalCase);
  ...
  const summary: HistoricalSimilaritySummary = {
    usable: matchedCaseIds.length >= minSampleSize,
