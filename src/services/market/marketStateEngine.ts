...
export class MarketStateEngine {
  ...
  update(state: MarketState): MarketInputs {
    ...
    // foreignDomesticFlowTone -1.6..1.6 -> participation 0..1
    const instRaw = clamp01((state.foreignDomesticFlowTone + 1.6) / 3.2);
    ...
    if (this.last) {
      const dPseiIndex = absPctDelta(this.last.pseiIndex, state.pseiIndex);
      const dPseiComposite = absPctDelta(this.last.pseiComposite, state.pseiComposite);
      const dFinancials = absPctDelta(this.last.financialsIndexValue, state.financialsIndexValue);

      // Convert deltas into “how consistent” (larger delta -> lower consistency)
      const deltaAvg = (dPseiIndex + dPseiComposite + dFinancials) / 3;
