...
import { sharedMarketServiceManager } from "../market/sharedMarketServiceManager";
...
export function useSharedMarketService(): MarketStreamSnapshot {
  const initial = useMemo<MarketStreamSnapshot>(() => {
    const snap = sharedMarketServiceManager.getSnapshot();
    return {
      snapshot: snap,
      connectionStatus: snap.connectionStatus,
    };
  ...
  useEffect(() => {
    const service = sharedMarketServiceManager.acquire();
    ...
    return () => {
      unsub();
      sharedMarketServiceManager.release();
    };
  }, []);
