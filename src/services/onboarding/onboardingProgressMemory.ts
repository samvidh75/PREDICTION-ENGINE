...
export type OnboardingProgress = {
  ...
  draft?: {
    ...
    // Optional onboarding preference: user-selected first sectors to explore.
    // Used only to drive discovery seeding (non-sensitive).
...
const VALID_AUTH_MODES: AuthMode[] = ["google", "email"];
...
// Must match src/services/discovery/discoveryIndex.ts sector titles.
const VALID_SECTOR_PREFERENCES = ["Banking", "IT", "FMCG", "Defence", "Pharma", "Energy"] as const;
...
function isValidSectorPreferences(raw: unknown): raw is string[] {
  if (!Array.isArray(raw)) return false;
  if (raw.length > 2) return false; // cap for low-friction UI
  return raw.every((x) => typeof x === "string" && (VALID_SECTOR_PREFERENCES as readonly string[]).includes(x));
}
  ...
  try {
    ...
    const safeDraft: OnboardingProgress["draft"] | undefined = hasDraft
      ? {
          ...
          sectorPreferences:
            draft &&
            "sectorPreferences" in draft &&
            isValidSectorPreferences((draft as { sectorPreferences?: unknown }).sectorPreferences)
              ? (draft as { sectorPreferences: string[] }).sectorPreferences
              : undefined,
        }
      : undefined;
