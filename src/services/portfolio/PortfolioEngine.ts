...
export interface UserHolding {
  ...
  sector: string;
  /** ISO date the position was first opened, if known — used only for an
   * approximate money-weighted return (XIRR) estimate. This model tracks
   * one averaged cost basis per symbol, not per-lot purchases, so XIRR
   * treats the whole position as a single cash outflow on this date. */
  buyDate?: string;
}
...
export function normalizeUserHolding(value: unknown): UserHolding | null {
  ...
  const buyDate = typeof candidate.buyDate === "string" && !Number.isNaN(Date.parse(candidate.buyDate))
    ? candidate.buyDate
    : undefined;
...
export class PortfolioEngine {
  ...
  public static addHolding(holding: UserHolding): boolean {
    ...
    if (existing) {
      ...
      if (normalized.buyDate && (!existing.buyDate || normalized.buyDate < existing.buyDate)) {
        existing.buyDate = normalized.buyDate;
      }
    } else {
