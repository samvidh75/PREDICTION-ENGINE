...
import { computeHealthometerCategoryEvaluation } from "../healthometer/healthometerCategoryEngine";
import { historicalHealthTimelineFromCategoryEvaluation, sectorRelativeStrengthNoteFromCategoryEvaluation } from "../healthometer/healthometerEngine";
...
export function buildNeuralMarketSynthesisEngine(inputs: NeuralMarketSynthesisInputs): NeuralMarketSynthesis {
  ...
  // Category-level Healthometer evaluation (educational + probabilistic; optional overlay support)
  const categoryEvaluation = computeHealthometerCategoryEvaluation({
    ...
    financial: corporateInputs.financial,
    sector:
      market?.marketInputs
        ? ({
            sectorMomentum: market.marketInputs.sectorMomentum,
            institutionalParticipation: market.marketInputs.institutionalParticipation,
            liquidityBreadth: market.marketInputs.liquidityBreadth,
          } as never)
        : ({
            sectorMomentum: 0.57,
            institutionalParticipation: 0.58,
  ...
  const historicalHealthTimeline = historicalHealthTimelineFromCategoryEvaluation(categoryEvaluation, narrativeKey);
  ...
  const timeline: NeuralSynthesisTimelineEntry[] = (() => {
    ...
    return base.concat(historicalHealthTimeline).concat(extra);
  })();
