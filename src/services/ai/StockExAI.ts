...
class StockExAI {
  ...
  async chat(userMessage: string): Promise<AIResponse> {
    ...
    if (this.isPortfolioQuery(lowerQuery)) {
   ...
   */
  private async handlePortfolioAnalysis(userMessage: string): Promise<AIResponse> {
...
4. **Conviction**: Medium (chart driven)`;
   ...
   */
  private async handleRecommendation(userMessage: string): Promise<AIResponse> {
    const stocks = ['HDFC', 'INFY', 'TCS', 'RELIANCE', 'MARUTI'];
...
= Balanced exposure`
   ...
   */
  private async handleMarketUpdate(userMessage: string): Promise<AIResponse> {
...
5. **Monitor**: Review every quarter, stay updated`;
   ...
   * Research Handler
   */
  private async handleResearch(userMessage: string): Promise<AIResponse> {
