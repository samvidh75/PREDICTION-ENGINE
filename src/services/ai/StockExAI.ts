/**
 * StockEx AI - ChatGPT of PSE Stock Market
...
class StockExAI {
  ...
  private async handlePortfolioAnalysis(_userMessage: string): Promise<AIResponse> {
    const stocks = ['BPI', 'JFC', 'TEL', 'BDO', 'ALI'];
...
✓ Build cash position for market dips`;
  ...
  private async handleTechnicalAnalysis(userMessage: string): Promise<AIResponse> {
    ...
    const symbol = stockMatch ? stockMatch[0] : 'TEL';
...
• Medium Conviction (65-80%): ${topBuys[1]?.symbol || 'TEL'} - Quality stock
• Base Case (60-65%): ${topBuys[2]?.symbol || 'ALI'} - Recovery play\n\n` +
...
💡 What it means:
• Shows how much investors pay per peso of annual earnings
...
📊 What it means:
• How much profit company makes from each peso of shareholder money
...
🇵🇭 PSE Dividend Stars:
BPI: 2-3% (steady payer)
MER: 4-5% (stable utility)
TEL: 4-6% (telecom dividend)
AP: 1-3% (holding co)
...
✓ BSP rate pause (inflation under control)
...
✓ Global sentiment improving\n\n` +
      ...
      `**Key Risks**
⚠️ Foreign fund flows volatile (geopolitical tensions)
...
Ask me "Analyze [SECTOR]" or "Compare [STOCK] vs [STOCK]"`;
  ...
  private async handleGreeting(userMessage: string): Promise<AIResponse> {
    ...
    for (const [key, value] of Object.entries(greetings)) {
      if (userMessage.toLowerCase().includes(key)) {
        return {
          response: value + '\n\n💡 Try: "Best PSE stocks" or "Analyze BPI"',
          confidence: 1.0,
          category: 'education'
        };
      }
  ...
  private async handleDefault(): Promise<AIResponse> {
    const response = `🤖 **StockEx AI - Your Personal Market Analyst**\n\n` +
      `I'm designed to be your ChatGPT for PSE stock market analysis.\n\n` +

      `**What I Can Do:**\n\n` +

      `📊 **Stock Analysis**
"Analyze BPI" - Deep fundamental dive
"Technical TEL" - Chart patterns & support/resistance
"Compare BDO vs JFC" - Head-to-head comparison\n\n` +
