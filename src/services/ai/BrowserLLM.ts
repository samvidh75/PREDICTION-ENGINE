...
class BrowserLLM {
  ...
  private getStockAnalysis(prompt: string): LLMResponse | null {
    ...
    if (query.includes('pe') || query.includes('price to earning')) {
      return {
        text: 'P/E Ratio = Price per share ÷ Earnings per share. Shows how much investors pay per peso of earnings. Low P/E doesn\'t mean cheap - check ROE, debt, and growth. PSE mid-caps average 15-20x P/E.',
        confidence: 0.95,
        tokens: 45,
      };
    }
