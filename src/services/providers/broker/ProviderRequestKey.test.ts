...
describe('ProviderRequestKey', () => {
  it('normalizes provider, operation, and symbol before hashing', () => {
    const a = buildRequestKey(' Yahoo ', 'quote', ' bdo.pse ', { range: '1d' });
    const b = buildRequestKey('yahoo', 'quote', 'BDO', { range: '1d' });

    expect(a).toEqual(b);
    expect(a.provider).toBe('pse');
    expect(a.operation).toBe('quote');
    expect(a.symbol).toBe('BDO');
  });
