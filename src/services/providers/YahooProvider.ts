 ...
 * a Philippine exchange label. Unknown values remain unavailable.
 *
 * Deliberately does NOT match Indian or Pakistan exchange labels.
 * A previous version treated them as PSE too, which would have silently
 * mislabeled a foreign quote as a real PSE one on any ticker
 * collision between the two markets.
 */
export function normalizeYahooExchange(exchangeName?: unknown, resolvedTicker?: string): 'PSE' | undefined {
  ...
  if (/\bpse\b|philippine stock exchange|philippines/.test(label)) return 'PSE';

  const ticker = (resolvedTicker || '').trim().toUpperCase();
  if (/\.PS$/.test(ticker)) return 'PSE';
  return undefined;
}
...
export class YahooProvider implements PriceProvider, MetadataProvider, HistoricalProvider, FinancialProvider {
  ...
  private normalizeSymbol(symbol: string): string {
    // If already suffixed with .PS, keep it.
    if (/\.(PS)$/i.test(symbol)) return symbol.toUpperCase();
  ...
  private async fetchJson(operation: 'quote' | 'metadata' | 'history', symbol: string, params: Record<string, unknown>, url: string): Promise<any> {
    const clean = symbol.replace(/\.(PS)$/i, '').trim().toUpperCase();
  ...
  async getQuote(symbol: string): Promise<StockQuote> {
    ...
    return {
      symbol: symbol.replace(/\.(PS)$/i, '').toUpperCase(),
  ...
  async getFinancials(symbol: string): Promise<YahooFinancials> {
    const cleanSym = symbol.replace(/\.(PS)$/i, '').toUpperCase();
