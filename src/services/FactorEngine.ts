...
export class FactorEngine {
  ...
  async calculateAndStoreFactors(symbol: string): Promise<StockFactorSnapshot[]> {
    ...
    const symbolInfo = symbolRes.rows[0] || { sector: null };
    // No fabricated fallback financials: when financial_snapshots has no row
    // for this symbol, these fields resolve to values that trigger each
    // formula's own existing "no data" neutral branch below (peRatio <= 0,
    // dividendYield <= 0, beta non-finite) rather than pretending a real
    // P/E of 25 or beta of 1.0 exists.
    const financial = financialsRes.rows[0] || { pe_ratio: 0, dividend_yield: 0, beta: NaN, eps: null };
