...
class NewsService {
  ...
  async refreshAllNews(): Promise<void> {
    ...
    const stocks = ['SM', 'BDO', 'JFC', 'AC', 'ALI', 'BPI', 'TEL', 'GLO', 'MER', 'MBT', 'URC', 'SMPH', 'PGOLD', 'RRHI', 'CEB', 'ICT', 'RLC'];
  ...
  private async fetchFromNewsAPI(symbol: string): Promise<NewsItem[]> {
    ...
    try {
      const query = `${symbol} stock market Philippines PSE`;
  ...
  private async fetchFromRSSFeeds(): Promise<NewsItem[]> {
    // Philippine financial news RSS feeds
    const rssFeeds = [
      { name: 'BusinessWorld', url: `https://www.bworldonline.com/feed/` },
      { name: 'Philstar Global', url: `https://www.philstar.com/rss/business` },
      { name: 'Manila Bulletin', url: `https://mb.com.ph/feed/business` },
    ];
