...
function sectorFromKeywords(text: string): SectorId[] {
  ...
  if (/(bank|banking|financial|bdo|bpi|metrobank)/i.test(t)) add("Financials");
  if (/(manufactur|industrial|conglomerate holdings|steel|cement)/i.test(t)) add("Industrial");
  if (/(holding firm|conglomerate|ayala corp|sm investments|jg summit)/i.test(t)) add("Holding Firms");
  if (/(property|real estate|land|condo|residential|ayala land|smph)/i.test(t)) add("Property");
  if (/(telecom|pldt|globe|utility|power|electric|meralco|services sector)/i.test(t)) add("Services");
  if (/(mining|oil|energy|refinery|nickel)/i.test(t)) add("Mining & Oil");
...
function detectIntent(text: string): { intent: AssistantIntent } {
  ...
  if (/(breadth|market breadth|participation|advance|decline|narrow|wide)/i.test(t)) return { intent: "breadth" };
  if (/(institution|foreign investor|foreign flow|institutional)/i.test(t)) return { intent: "institutional" };
