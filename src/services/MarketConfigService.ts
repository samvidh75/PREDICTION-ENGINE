...
export class MarketConfigService {
  ...
  async getMarketStatus(): Promise<MarketStatus> {
    ...
    if (dayOfWeek === 0 || dayOfWeek === 6) {
      return {
        ...
        nextOpenTime: this.nextOpen(pht, holidays),
    ...
    if (totalMinutes >= this.MARKET_OPEN && totalMinutes < this.MARKET_CLOSE) {
      return {
        ...
        message: `Market open (closes at 3:30 PM PHT)`,
      };
    }
    ...
    if (totalMinutes >= this.PREOPEN && totalMinutes < this.MARKET_OPEN) {
      return {
        ...
        message: `Pre-open session (opens at 9:30 AM PHT)`,
      };
    }
  ...
  async getDataFreshnessMessage(): Promise<string> {
    const status = await this.getMarketStatus();
    if (status.isOpen) return `Live prices (market open until 3:30 PM PHT)`;
    ...
    if (snapshot) {
      ...
      return `Market closed. Snapshot from ${t.toLocaleTimeString('en-PH')} PHT`;
    }
  ...
  private getDefaultHolidays(): string[] {
    return [
      '2026-01-01', '2026-04-09', '2026-04-10', '2026-05-01',
      '2026-06-12', '2026-08-21', '2026-11-30', '2026-12-25',
    ];
  }
