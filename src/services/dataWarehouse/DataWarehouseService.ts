...
// bulk fundamentals dataset to screen the universe against.
//
// What *is* real and bulk-available is the bundled stock-universe.json
// (StockUniverseAdapter): symbol, company name, sector/subsector and listing
// date scraped from PSE Edge's company directory, plus market cap (millions
// PHP) scraped from stockanalysis.com — ~282 PSE-listed names, regenerated by
// scripts/generate-stock-universe.ts.
//
// The 0-100 factor scores (quality, valuation, growth, momentum, risk, health,
// riskAdjusted) are NOT currently populated. An earlier revision of this
// comment claimed they were "computed from real fundamentals+price data at
// generation time"; they were not — every entry carried a hardcoded 50, so
// every screener result ranked identically. The generator now omits the scores
// object rather than emit a placeholder, and loadUniverseRows() below maps the
// absent values to null. Those metrics stay in the catalog so the shape is
// stable, but they will filter/sort as null until a real batch scoring
// pipeline exists. Do not reintroduce a default value here.
export class DataWarehouseService {
  ...
  private applyGrouping(
    ...
    for (const [, group] of groups) {
      ...
      for (const m of measures) {
        ...
        // null, not 0, when the group has no values for this measure — a 0 here
        // reads as a real "this sector scores zero" rather than "not measured",
        // which matters now that factor scores are legitimately absent.
        aggregated[m] = values.length > 0 ? values.reduce((a, b) => a + b, 0) / values.length : null;
      }
  ...
  /**
   * Loads the real bundled stock universe (~282 PSE-listed names) and flattens
