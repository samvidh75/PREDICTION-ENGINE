 ...
 *   - Reports EMPTY_RESPOPSE for non-existent symbols
 */
...
describe('PriceRealAdapter', () => {
  ...
  it('returns EMPTY_RESPOPSE for symbols not in the database', async () => {
    ...
    if (!result.ok) {
      expect(['EMPTY_RESPOPSE', 'ADAPTER_UNAVAILABLE', 'UNKNOWN_ADAPTER_ERROR']).toContain(result.errorCode);
    }
  });

  it('returns EMPTY_RESPOPSE for intraday (not available from DB)', async () => {
  ...
  it('normalises symbols with exchange prefixes', async () => {
    const result = await adapter.getDailyCandles('PSE:RELIANCE');
    if (result.ok) {
