...
export interface RegistryEntry {
  ...
  industry: string;
  exchange: string;
  /** Founding year — populated only where independently well-documented public record; absent otherwise. */
  founded?: string;
  /** Primary corporate HQ city — populated only where confidently verified; absent otherwise. */
  headquarters?: string;
  marketCap?: number;
  isin?: string;
  bseCode?: string;
...
const VERIFIED_REGISTRY: RegistryEntry[] = [
  ...
  },
  {
    ...
    exchange: 'PSE',
    founded: '1935',
    headquarters: 'Taguig City, Metro Manila',
  ...
  },
  {
    ...
    exchange: 'PSE',
    founded: '2003',
    headquarters: 'Parañaque City, Metro Manila',
  ...
  },
  {
    ...
    exchange: 'PSE',
    founded: '2007',
    headquarters: 'Muntinlupa City, Metro Manila',
...
/* A prior version of this file padded the registry with 450 fabricated
   "Philippine X Company N" shell entries (fake names, random market caps) to
   pad the search universe. Removed — this registry only holds real, manually
   verified PSE-listed companies. Anything not in VERIFIED_REGISTRY is
   genuinely absent rather than backfilled with invented data. */
const ALL_REGISTRY = [...VERIFIED_REGISTRY];
