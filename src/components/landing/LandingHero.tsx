...
export const LandingHero: React.FC<LandingHeroProps> = ({
  ...
  return (
    <div className="relative w-full max-w-5xl mx-auto flex flex-col items-center justify-center text-center select-none py-12 md:py-20">
      ...
      <div
        className={`flex items-center gap-2 px-3 py-1.5 rounded-full border border-neutral-200/60 bg-white/70 shadow-[0_2px_10px_rgba(0,0,0,0.01)] backdrop-blur-sm transition-all duration-700 transform ${
      ...
      >
        ...
        <span className="text-[10px] tracking-[0.2em] font-mono font-semibold text-neutral-500 uppercase">
      ...
      {/* Main Title Heading */}
      <h1
        className={`mt-6 text-4xl md:text-6xl font-semibold tracking-tight text-neutral-900 leading-tight transition-all duration-1000 delay-100 transform ${
          mounted ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"
        }`}
      >
      ...
      {/* Narrative Subtitle */}
      <p
        className={`mt-6 text-[15px] md:text-[16px] text-neutral-500 max-w-2xl leading-relaxed transition-all duration-1000 delay-200 transform ${
          mounted ? "translate-y-0 opacity-100" : "translate-y-4 opacity-0"
        }`}
      >
      ...
      >
        <button
          ...
          className={`group relative w-full h-[48px] rounded-md text-[13px] font-semibold uppercase tracking-wider transition-all duration-300 ${
            ...
            isConnecting
              ? "bg-neutral-900 text-neutral-400 cursor-not-allowed border border-neutral-800"
              : error
              ? "hover:bg-magenta-50"
      ...
      >
        {[
          ...
          <div key={item.label} className="flex flex-col items-center">
            ...
            <span className="mt-1 text-[12px] font-semibold text-neutral-700 font-mono tracking-wide">
              {item.value}
            </span>
          </div>
        ))}
