 ...
 * holdings or silent neutral defaults.
 */
...
export default function PortfolioDoctor() {
  ...
  if (state === 'error') {
    return <StatePanel title="Analysis Failed" message={error} tone="error" />;
  }

  if (state === 'unavailable') {
    return <StatePanel title="Portfolio Analysis Unavailable" message={error} tone="unavailable" />;
  }
  ...
  if (state === 'empty' || !data) {
    return (
      <StatePanel
        ...
        message={error || "Add stocks to your portfolio to see the Doctor's analysis."}
        tone="empty"
      />
    );
  }
    ...
    <div className="space-y-6">
      <div className="border-4 border-black bg-yellow-300 p-4" style={{ boxShadow: '6px 6px 0px #000' }}>
        <div className="flex flex-wrap items-center gap-3">
          ...
          {data.isDemo && (
            <span className="border-2 border-black bg-white px-2 py-1 text-xs font-extrabold uppercase">Demo</span>
          )}
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <MetricCard
          ...
          value={data.completenessScore === null ? 'N/A' : `${Math.round(data.completenessScore)}%`}
        />
      </div>
        ...
        {factorEntries.length > 0 ? (
          factorEntries.map(([key, value]) => (
            <Bar
              ...
              value={value}
              color={toPercent(value) >= 60 ? 'bg-green-500' : toPercent(value) >= 40 ? 'bg-amber-400' : 'bg-red-400'}
            />
          ))
        ) : (
      ...
      {data.neutralizedFields.length > 0 && (
        <div className="border-4 border-black bg-amber-100 p-5" style={{ boxShadow: '6px 6px 0px #000' }}>
          ...
          <p className="text-sm mb-3">
            Some factor inputs were unavailable and were neutralized by the portfolio engine. Review these holdings before acting on the analysis.
          </p>
          <ul className="space-y-1 text-sm">
            {data.neutralizedFields.map((entry) => (
              <li key={entry.symbol}>
                <strong>{entry.symbol}:</strong> {entry.neutralizedFactors.join(', ') || 'unspecified factors'}
              </li>
            ))}
          </ul>
        </div>
...
function StatusCard({ label, status }: { label: string; status: string }) {
  return (
    <div className="border-4 border-black bg-blue-100 p-3 text-center" style={{ boxShadow: '4px 4px 0px #000' }}>
      <p className="text-xs font-bold uppercase text-gray-500">{label}</p>
