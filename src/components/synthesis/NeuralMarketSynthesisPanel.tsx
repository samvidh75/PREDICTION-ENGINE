...
function plainInvestingCopy(text: string): string {
  return text
    .replace(/institutional/gi, "large-investor")
    .replace(/neural/gi, "market")
    .replace(/telemetry/gi, "data")
    .replace(/hologram/gi, "visual")
    .replace(/calibration/gi, "adjustment")
    .replace(/\bnode\b/gi, "point");
}
...
export default function NeuralMarketSynthesisPanel({ compact = false }: PanelProps): JSX.Element {
  ...
  const narrativeBody = plainInvestingCopy(synthesis.narrative.cinematicBody.replace(/cinematic/gi, "calm"));
  const conditionsNote = plainInvestingCopy(synthesis.narrative.conditionsNote.replace(/cinematic/gi, "calm"));
...
<div className="text-[12px] uppercase tracking-[0.18em] text-white/70">Market Synthesis</div>
          <div className="mt-3 text-[22px] font-medium text-white/92">
            {compact ? "Market insight summary" : "Market insight summary"}
          </div>
        </div>
      ...
      <div className="rounded-[24px] border border-white/10 bg-black/20 backdrop-blur-[24px] p-6 shadow-[0_0_40px_rgba(0,0,0,0.35)]">
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
          <div className="md:col-span-2">
            ...
            <div className="mt-2 text-[18px] leading-[1.6] text-white/92 font-semibold">{plainInvestingCopy(synthesis.narrative.editorialHeadline)}</div>
          ...
          <div>
            ...
            <div className="mt-3 text-[13px] leading-[1.7] text-white/80">{plainInvestingCopy(synthesis.healthometer.rationale)}</div>
            <div className="mt-3 rounded-[18px] border border-white/10 bg-black/25 p-4">
              ...
              <div className="mt-2 text-[13px] leading-[1.6] text-white/85">{plainInvestingCopy(synthesis.healthometer.confidenceMarginText)}</div>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
          <div className="rounded-[22px] border border-white/10 bg-black/25 p-5">
            <div className="text-[12px] uppercase tracking-[0.18em] text-white/60">Macro and policy view</div>
            <div className="mt-2 text-[14px] font-semibold text-white/92">{plainInvestingCopy(synthesis.macroGeopolitical.headline)}</div>
            <div className="mt-3 text-[13px] leading-[1.8] text-white/80">{plainInvestingCopy(synthesis.macroGeopolitical.body)}</div>
            <div className="mt-3 h-[1px]" style={{ background: "rgba(255,255,255,0.08)" }} />
            <div className="mt-3 text-[13px] leading-[1.8] text-white/80">
              <span className="text-white/70">Liquidity view:</span> {plainInvestingCopy(synthesis.liquidityIntelligenceCore)}
            </div>
          </div>

          <div className="rounded-[22px] border border-white/10 bg-black/25 p-5">
            <div className="text-[12px] uppercase tracking-[0.18em] text-white/60">Investor behaviour summary</div>
            <div className="mt-3 text-[13px] leading-[1.8] text-white/80">{plainInvestingCopy(synthesis.institutionalBehaviour)}</div>
            <div className="mt-3 text-[13px] leading-[1.8] text-white/80">{plainInvestingCopy(synthesis.behaviouralPsychology)}</div>
            <div className="mt-3 text-[13px] leading-[1.8] text-white/80">{plainInvestingCopy(synthesis.sectorRotationMatrix)}</div>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-3">
          <div className="lg:col-span-1 rounded-[22px] border border-white/10 bg-black/25 p-5">
            ...
            <div className="mt-3 text-[13px] leading-[1.8] text-white/85">{plainInvestingCopy(synthesis.futureProbabilityFramework)}</div>
          </div>
            ...
            <div className="mt-4 space-y-3">
              {timeline.map((t) => (
                <div key={t.id} className="rounded-[18px] border border-white/10 bg-black/20 p-4">
                  ...
                  <div className="mt-2 text-[13px] leading-[1.7] text-white/85">{plainInvestingCopy(t.text)}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="lg:col-span-1 rounded-[22px] border border-white/10 bg-black/25 p-5">
            <div className="text-[12px] uppercase tracking-[0.18em] text-white/60">Market scanner</div>
            <div className="mt-4 space-y-3">
              {scanners.map((c) => (
                <div key={c.id} className="rounded-[18px] border border-white/10 bg-black/20 p-4">
                  <div className="text-[10px] uppercase tracking-[0.18em] text-white/55">{plainInvestingCopy(c.title)}</div>
                  <div className="mt-2 text-[13px] leading-[1.7] text-white/85">{plainInvestingCopy(c.body)}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="mt-6 text-[12px] uppercase tracking-[0.18em] text-white/45">
          Calm learning context • clear boundaries
        </div>
      </div>
    </section>
  );
