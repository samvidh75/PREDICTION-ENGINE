...
function seiSafeLine(): string {
  return "Calm learning boundaries | probabilistic context";
}
...
function limitWords(str: string, maxWords: number): string {
  ...
  const structuralCard = (
    <CompanyUniverseCard className="p-6">
      ...
      <div className="mt-4 text-[14px] leading-[1.9] text-white/80">{limitWords(strategicSummary, 120)}</div>
  ...
  const institutionalMacroCard = (
    <CompanyUniverseCard className="p-6">
      <div className="text-[12px] uppercase tracking-[0.18em] text-white/60">Investor and macro context</div>
      <div className="mt-3 text-[16px] font-semibold leading-[1.3] text-white/92">{plainInvestingCopy(synthesis.macroGeopolitical.headline)}</div>
      <div className="mt-3 text-[13px] leading-[1.8] text-white/80">{plainInvestingCopy(synthesis.macroGeopolitical.body)}</div>

      <div className="mt-5 rounded-[22px] border border-white/10 bg-black/25 p-4">
        <div className="text-[10px] uppercase tracking-[0.18em] text-white/55">Large-investor participation view</div>
        <div className="mt-2 text-[13px] leading-[1.7] text-white/85">{plainInvestingCopy(synthesis.institutionalBehaviour)}</div>
      </div>
    </CompanyUniverseCard>
  );
      ...
      <div className="mt-3 space-y-4">
        {capsuleShown.map((c, idx) => (
          <div key={c.id} className="rounded-[18px] border border-white/10 bg-black/20 p-4">
            ...
            <div className="mt-3 text-[11px] uppercase tracking-[0.18em] text-white/45">Educational probability • calm boundaries</div>
          </div>
        ))}
      </div>
    </CompanyUniverseCard>
        ...
        <div className="mx-auto max-w-[1680px]">
          <div className="mb-6 flex items-end justify-between gap-4">
            ...
            <div className="text-[11px] uppercase tracking-[0.18em] text-white/45">
              {beginner ? "Beginner clarity | simplified density" : "Full context | detailed view"} | state: {confidenceState}
            </div>
          </div>
