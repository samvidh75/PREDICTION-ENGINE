...
import { backgroundThrottleController, type BackgroundState } from "../../services/realtime/backgroundThrottleController";
...
export default function useCompanyLiveTelemetry(args: {
  ...
  const priceRef = useRef<number>(initial.price);
  ...
  const [backgroundState, setBackgroundState] = useState<BackgroundState>(() => backgroundThrottleController.getState());

  useEffect(() => {
    const unsubRoute = routeIntensityStore.subscribe((i) => setRouteIntensity(i));
    const unsubBg = backgroundThrottleController.subscribe((s) => setBackgroundState(s));
    return () => {
      unsubRoute();
      unsubBg();
    };
  ...
  useEffect(() => {
    if (!enabled) return;
    if (backgroundState === "hidden") return;

    const scale = routeIntensity === "high" ? 1 : routeIntensity === "medium" ? 1.8 : 3.5;
    const effectiveTickMs = Math.max(450, Math.round(tickMs * scale));

    anchorPriceRef.current = initial.price;
    priceRef.current = initial.price;
    ...
    const tick = () => {
      ...
      if (anchorAgeMs > 45_000) {
        ...
        anchorPriceRef.current = priceRef.current;
      }
      ...
      setSnap((prev) => {
        ...
        // Keep an up-to-date ref for anchor computations.
        ...
        const bidDepth = clamp(
          0.28 + liqPart * 0.42 + (Math.sin(t * 0.7 + seed) * 0.06) - (volEnv - 0.4) * 0.04,
          0.05,
          0.98,
        );
        const askDepth = clamp(
          0.26 + liqPart * 0.40 + (Math.cos(t * 0.52 + seed) * 0.06) + (volEnv - 0.4) * 0.05,
          0.05,
          0.98,
        );
    ...
    const intervalId = window.setInterval(tick, effectiveTickMs);

    return () => {
      window.clearInterval(intervalId);
    };
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [enabled, tickMs, routeIntensity, backgroundState, ticker, confidenceState, companyHealthState, seed, initial.price]);
