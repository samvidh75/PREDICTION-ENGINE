...
export const RangeInfographic: React.FC<RangeInfographicProps> = ({
  ...
  return (
    <div className="flex flex-col space-y-6 select-none">
      ...
      <div className="bg-[#0D1117] border border-[rgba(148,163,184,0.16)] p-6 rounded-none flex flex-col space-y-4">
        ...
        <div className="relative pt-4 pb-2">
          {/* Tracking line */}
          <div className="h-1 bg-[rgba(148,163,184,0.08)] relative rounded-none w-full">
        ...
        <div className="flex justify-between items-center font-mono text-xs text-[#9AA7B5]">
          <div className="flex flex-col">
            <span className="text-[9px] uppercase text-[#64748B]">52W Low</span>
            <span>₹{low52Week.toLocaleString("en-IN")}</span>
          </div>
          <div className="flex flex-col text-right">
            <span className="text-[9px] uppercase text-[#64748B]">52W High</span>
        ...
        <div className="grid grid-cols-5 gap-2">
          {(Object.keys(performance) as Array<keyof typeof performance>).map((key) => {
            ...
            return (
              <div
                key={key}
                className="p-2 border border-[rgba(148,163,184,0.16)] bg-[#0D1117] flex flex-col items-center text-center font-mono"
              >
                <span className="text-[9px] text-[#64748B]">{key}</span>
      ...
      <div className="w-full aspect-[16/10] bg-[#0D1117] border border-[rgba(148,163,184,0.16)] rounded-none relative flex flex-col p-6 overflow-hidden">
        <div className="flex items-center justify-between border-b border-[rgba(148,163,184,0.16)] pb-2 mb-4">
          <div className="flex items-center space-x-1.5">
            <AreaChart className="w-4 h-4 text-[#06B6D4]" />
            <span className="text-[11px] font-mono tracking-wider font-semibold text-[#E6EDF3]">
        ...
        <div className="flex-1 flex items-end justify-between gap-3 pt-6 font-mono text-[9px] text-[#9AA7B5]">
          {[
            ...
            <div key={bar.label} className="flex-1 flex flex-col items-center space-y-2">
              <span className="text-[8px] text-[#64748B]">{bar.amount}</span>
              <div
                className="w-full bg-[#0D1117] border border-[rgba(148,163,184,0.16)] hover:bg-[#06B6D4]/5 hover:border-[#06B6D4]/30 transition-all duration-300 relative group"
              ...
              <span className="text-[#64748B] font-bold uppercase">{bar.label}</span>
            </div>
          ))}
        </div>
      </div>
