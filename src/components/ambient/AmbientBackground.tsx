...
export default function AmbientBackground(): JSX.Element {
  ...
  // Step 7: semantic neon mapping (stock-state → semantic glow)
  // Values come from AdaptiveColourCoordinationLayer via CSS vars.
  const fogCyan = "var(--ss-sem-neon-glow-strength)";
  const fogMagenta = "var(--ss-sem-neon-glow-drift)";
  const fogDeepBlue = "var(--ss-sem-neon-glow-structural)";
  ...
  return (
    <div className="absolute inset-0 z-0 overflow-hidden">
      ...
      <div
        ...
        style={{
          background:
            `radial-gradient(ellipse at 20% 10%, var(--ss-sem-neon-glow-structural), transparent 55%), radial-gradient(ellipse at 80% 30%, var(--ss-sem-neon-glow-drift), transparent 60%), radial-gradient(ellipse at 50% 80%, var(--ss-sem-neon-glow-strength), transparent 58%), #020304`,
        }}
      />
      ...
      <div className="absolute inset-0 z-[2]">
        {particles.map((p) => {
          const color =
            p.hue === "cyan"
              ? "var(--ss-sem-neon-glow-strength)"
              : p.hue === "magenta"
                ? "var(--ss-sem-neon-glow-drift)"
                : p.hue === "deepBlue"
                  ? "var(--ss-sem-neon-glow-structural)"
                  : "var(--ss-sem-neon-glow-uncertainty)";
