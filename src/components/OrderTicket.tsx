...
export const OrderTicket: React.FC<OrderTicketProps> = ({
  ...
  return (
    <div className={`bg-white border rounded-none p-6 relative overflow-hidden shadow-[0_4px_20px_rgba(0,0,0,0.01)] transition-all duration-300 ${
      marginError ? "border-[#D946EF]" : "border-[#E5E5E5]"
    } select-none`}>
      ...
      <form onSubmit={handleExecute} className="space-y-4">
        {/* Toggle Switch */}
        <div className="grid grid-cols-2 h-11 border border-[#E5E5E5] bg-[#FAFAFA]">
          <button
            type="button"
            ...
            className={`w-full h-full flex items-center justify-center font-bold text-xs uppercase transition-colors rounded-none ${
              orderType === "BUY" ? "bg-[#0A0A0A] text-white" : "text-[#525252]"
            }`}
          >
            Buy Position
          </button>
          <button
            ...
            className={`w-full h-full flex items-center justify-center font-bold text-xs uppercase transition-colors rounded-none ${
              orderType === "SELL" ? "bg-[#0A0A0A] text-white" : "text-[#525252]"
            }`}
          >
            Sell Position
          </button>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="flex flex-col space-y-1">
            <label className="text-[9px] uppercase font-mono text-neutral-400">Share Quantity</label>
            <input
              type="number"
              min={1}
              value={quantity}
              ...
              className="font-mono text-sm tracking-tight text-[#0A0A0A] bg-white border border-[#E5E5E5] h-11 px-4 focus:border-[#06B6D4] outline-none rounded-none"
            />
          </div>

          <div className="flex flex-col space-y-1">
            <label className="text-[9px] uppercase font-mono text-neutral-400">Limit Price (₹)</label>
            <input
              type="number"
              min={1}
              value={price}
        ...
        {/* Exposure math */}
        <div className="bg-[#FAFAFA] border border-[#E5E5E5] p-3 flex justify-between items-center font-mono text-xs text-neutral-500">
