export function getScoreColor(s: number) {
  if (s >= 80) return '#0066cc';
  if (s >= 65) return '#0071e3';
...
export function getSignalFromScore(s: number | null) {
  ...
  if (s >= 80) return { text: 'Strong Buy ↗', color: '#0066cc' };
  if (s >= 65) return { text: 'Buy ↗', color: '#0066cc' };
...
export function ConfidenceRing({ pct, size = 32 }: { pct: number; size?: number }) {
  ...
  return (
    <svg width={size} height={size}>
      ...
      <circle cx={size / 2} cy={size / 2} r={r} fill="none" stroke="#0066cc" strokeWidth={3}
              strokeLinecap="round" strokeDasharray={`${filled} ${circ}`}
              transform={`rotate(-90 ${size / 2} ${size / 2})`}
              style={{ transition: 'stroke-dasharray 0.8s ease' }} />
      <text x={size / 2} y={size / 2} textAnchor="middle" dominantBaseline="middle"
            fontSize={size * 0.28} fontWeight="600" fontFamily="system-ui, -apple-system, sans-serif" fill="#1d1d1f">
        {Math.round(pct)}
      </text>
    </svg>
  );
...
export function MiniSparkline({
  data, color = '#0066cc', width = 60, height = 26
