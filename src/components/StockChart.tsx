...
export default function StockChart({
  ...
  useEffect(() => {
    const loadChart = async () => {
      try {
        ...
        const { createChart, CandlestickSeries, LineSeries } = module;
        ...
        // Add candlestick series (v5 API: addSeries(definition, options))
        ...
        // Add VWAP if selected (cumulative volume-weighted average price
        // over the displayed session, derived from real OHLCV candles).
        if (selectedIndicators.has('VWAP') && showIndicators) {
          const vwapSeries = chart.addSeries(LineSeries, {
            color: '#a78bfa',
            lineWidth: 2,
            lineStyle: 1, // dashed
          });
          vwapSeries.setData(calculateVWAP(ohlcData) as any);
        }
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        {showIndicators && (
          <div style={{ display: 'flex', gap: '8px' }}>
            {['SMA20', 'EMA12', 'VWAP', 'RSI', 'MACD'].map((indicator) => (
              <button
                key={indicator}
      ...
      {/* Chart Container — always mounted so the ref exists before the chart-creation effect runs */}
      <div style={{ position: 'relative' }}>
        <div ref={containerRef} style={{ width: '100%', height: `${height}px`, visibility: isLoading || error ? 'hidden' : 'visible' }} />
        {isLoading && (
          <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: colors.textSecondary }}>
            Loading chart...
          </div>
        )}
        {error && (
...
/**
 * Calculate VWAP (Volume-Weighted Average Price) — a cumulative, session-style
 * VWAP derived from real OHLCV candles:
 *
 *    PV = Σ typical-price × volume
 *    TV = Σ volume
 *    VWAP = PV / TV   (typical price = (high + low + close) / 3)
 *
 * Computed over the displayed window so it stays meaningful regardless of
 * whether the data is intraday or daily EOD.
 */
function calculateVWAP(data: OHLC[]): Array<{ time: string; value: number }> {
  const vwap: Array<{ time: string; value: number }> = [];
  let cumulativePV = 0;
  let cumulativeTV = 0;
  ...
  for (const candle of data) {
    ...
    cumulativeTV += candle.volume;
    if (cumulativeTV > 0) {
      vwap.push({ time: candle.time, value: cumulativePV / cumulativeTV });
    }
  }
