  ...
  switch (kind) {
    case "stock_movement":
      return {
        ...
        value:
          state === "ELEVATED_RISK"
            ? `Drift ${jitterPct(0.4, 0.55).toFixed(2)}% • elevated volatility`
            : state === "MOMENTUM_WEAKENING"
              ? `Drift ${jitterPct(0.2, 0.45).toFixed(2)}% • selective trends`
              : `Drift ${jitterPct(0.1, 0.35).toFixed(2)}% • steady trend`,
      };
    case "liquidity_behavior":
      return {
        ...
        value:
          state === "ELEVATED_RISK"
            ? `Depth ${jitterPct(0.48, 0.15).toFixed(2)} • low liquidity`
            : state === "MOMENTUM_WEAKENING"
              ? `Depth ${jitterPct(0.56, 0.14).toFixed(2)} • moderating volume`
              : `Depth ${jitterPct(0.62, 0.12).toFixed(2)} • stable volume`,
      };
    case "institutional_participation":
      return {
        ...
        value:
          state === "ELEVATED_RISK"
            ? `Activity ${jitterPct(0.58, 0.18).toFixed(2)} • risk-averse`
            : state === "MOMENTUM_WEAKENING"
              ? `Activity ${jitterPct(0.64, 0.14).toFixed(2)} • slower accumulation`
              : `Activity ${jitterPct(0.70, 0.10).toFixed(2)} • steady accumulation`,
      };
    case "sector_rotation":
      return {
        ...
        value:
          state === "MOMENTUM_WEAKENING"
            ? `Rotation ${jitterPct(0.44, 0.20).toFixed(2)} • concentrated focus`
            : state === "ELEVATED_RISK"
              ? `Rotation ${jitterPct(0.48, 0.22).toFixed(2)} • defensive positioning`
              : `Rotation ${jitterPct(0.52, 0.18).toFixed(2)} • constructive trends`,
      };
    case "confidence_conditions":
      return {
        title: "Sentiment conditions",
        value:
          state === "CONFIDENCE_RISING"
            ? `Sentiment ${jitterPct(0.78, 0.14).toFixed(2)} • improving`
            : state === "STABLE_CONVICTION"
              ? `Sentiment ${jitterPct(0.70, 0.12).toFixed(2)} • stable`
              : state === "NEUTRAL_ENVIRONMENT"
                ? `Sentiment ${jitterPct(0.62, 0.12).toFixed(2)} • balanced`
                : state === "MOMENTUM_WEAKENING"
                  ? `Sentiment ${jitterPct(0.56, 0.12).toFixed(2)} • selective`
                  : `Sentiment ${jitterPct(0.50, 0.12).toFixed(2)} • cautious`,
      };
    ...
    default:
      return {
        ...
        value:
          state === "ELEVATED_RISK"
            ? `Sensitivity ${jitterPct(0.73, 0.18).toFixed(2)} • responsive to stress`
            : state === "MOMENTUM_WEAKENING"
              ? `Sensitivity ${jitterPct(0.58, 0.16).toFixed(2)} • selective sensitivity`
              : `Sensitivity ${jitterPct(0.52, 0.14).toFixed(2)} • contained sensitivity`,
      };
  }
}
