...
export default function HolographicTelemetryEngine({
  ...
  const telemetryHeaderQaOverride =
    typeof window !== "undefined" &&
    new URLSearchParams(window.location.search).get("qaTelemetryHeader") === "1";

  const shouldShowHeader = showHeader || telemetryHeaderQaOverride;
    ...
    <section className="relative">
      {shouldShowHeader && (
        <div className="mb-4 flex items-end justify-between gap-4">
          <div className="min-w-0">
            <ModuleKicker>{title}</ModuleKicker>
            <ModuleTitle>Structured market signals</ModuleTitle>
          </div>
            ...
            >
              ...
              <NavLabel>{labelForConfidenceState(state)}</NavLabel>
            </div>
        </div>
      )}
      ...
      >
        ...
        <div className="absolute inset-0 z-[3] pointer-events-none">
          {visibleReadouts.map((r) => {
            ...
            return (
              ...
              >
                <MetricLabel>{confidenceToneLabel(r)}</MetricLabel>
                <MetricSubValue className="mt-2">{r.valueLabel}</MetricSubValue>
        ...
        <div className="absolute left-4 bottom-3 z-[4] pointer-events-none">
          <NavLabel className="text-white/45">Educational signal context • calm and clear</NavLabel>
        </div>
      </div>
    </section>
  );
