...
export type IndexQuote = {
  ...
  low?: string;
  high?: string;
  /** position of last price inside the day range, 0–100. Omitted when no
      real level data exists — the design's range rail is only drawn when a
      real level/range is available, never filled with a guess. */
  posPct?: number;
};
...
export function MarketTape({
  ...
  const hasRange = (ix: IndexQuote) =>
    typeof ix.posPct === "number" && ix.low !== undefined && ix.high !== undefined;
