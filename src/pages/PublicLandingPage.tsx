...
function LegendDot({ color, label, value }: { color: string; label: string; value: string }) {
  return <div className="flex items-center gap-1.5 text-[12px]"><div className="w-2 h-2 rounded-full" style={{ background: color }} /><span className="text-[#7a7a7a]">{label}</span><span className="font-[600] text-[#1d1d1f]">{value}</span></div>;
}
    ...
    <AppShell active="research">
      ...
      <div className="bg-white">
        <div className="max-w-[980px] mx-auto px-6 py-[80px] text-center">
          <div className="inline-flex items-center gap-1.5 bg-[#f5f5f7] text-[#0066cc] text-[12px] font-[600] px-3 py-1 rounded-[9999px] mb-6">
            <Sparkles size={13} /> AI-POWERED STOCK INTELLIGENCE
          </div>
          <h1 className="text-apple-hero mb-3">Understand businesses.<br />Invest better.</h1>
          ...
          <div className="flex items-center justify-center gap-6 text-[14px] text-[#7a7a7a]">
            {['No credit card required', 'Cancel anytime', 'Trusted by 2M+ investors'].map(t => (
              <div key={t} className="flex items-center gap-1.5"><Check size={13} className="text-[#0066cc]" />{t}</div>
            ))}
          </div>
        </div>
      </div>

      {/* DARK TILE — Stock preview */}
      <div className="bg-[#272729]">
        <div className="max-w-[980px] mx-auto px-6 py-[80px]">
          <div className="grid grid-cols-2 gap-12 items-center">
            <div>
              <p className="text-[12px] font-[600] text-[#2997ff] tracking-[-0.12px] mb-2">LIVE PREVIEW</p>
            ...
            <div className="bg-[#2a2a2c] rounded-[18px] p-6 border border-[#333]">
              ...
              <div className="flex items-center gap-5 mb-4">
                {loading ? <div className="w-[88px] h-[88px] rounded-full bg-[#333] animate-pulse flex-shrink-0" />
              ...
              <div className="flex items-center justify-between pt-3 border-t border-[#333]">
                ...
                <button onClick={() => productNavigate("stock", "HDFCBANK")}
                  className="text-[12px] font-[600] text-[#2997ff] flex items-center gap-1 active:scale-[0.95]">
      ...
      <div className="bg-[#f5f5f7]">
        <div className="max-w-[980px] mx-auto px-6 py-[80px] text-center">
          ...
          <h2 className="text-[40px] font-[600] text-[#1d1d1f] leading-[1.1] mb-6">Today's research highlight</h2>
          <div className="bg-white border border-[#e0e0e0] rounded-[18px] p-6 max-w-[600px] mx-auto text-left">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2 text-[12px] font-[600] text-[#0066cc]"><Sparkles size={14} /> AI Insight</div>
              <span className="text-[11px] text-[#7a7a7a]">Generated today</span>
            </div>
            <div className="text-[17px] font-[600] text-[#1d1d1f] leading-[1.24] tracking-[-0.374px] mb-2">HDFCBANK: Compounding moat remains intact</div>
            ...
            <div className="flex items-center gap-2 text-[12px] font-[600] text-[#0066cc]">
              ...
              <button onClick={() => productNavigate("stock", "HDFCBANK")} className="ml-auto flex items-center gap-1 active:scale-[0.95]">Read Full Thesis <ArrowRight size={12} /></button>
            </div>
          </div>
        </div>
      </div>

      {/* WHITE TILE — Market + Performance */}
      <div className="bg-white">
        <div className="max-w-[980px] mx-auto px-6 py-[80px]">
          <div className="grid grid-cols-2 gap-5">
            <div className="bg-[#f5f5f7] rounded-[18px] p-6 border border-[#e0e0e0]">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2"><span className="text-[14px] font-[600] text-[#1d1d1f]">Market Overview</span><span className="text-[10px] text-[#0066cc] font-[600] bg-white px-2 py-0.5 rounded-[9999px]">Live</span></div>
                <div className="flex gap-1">
                  {['1D', '1W', '1M', 'YTD', '1Y'].map(p => (
                    <button key={p} className={`text-[10px] font-[500] px-2 py-0.5 rounded-[5px] active:scale-[0.95] ${p === '1D' ? 'bg-[#1d1d1f] text-white' : 'text-[#7a7a7a]'}`}>{p}</button>
                  ))}
                </div>
              </div>
              <ResponsiveContainer width="100%" height={80}>
                <AreaChart data={chartData}>
                  <defs><linearGradient id="b1" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#0066cc" stopOpacity={0.1} /><stop offset="95%" stopColor="#0066cc" stopOpacity={0} /></linearGradient></defs>
                  <Area type="monotone" dataKey="v" stroke="#0066cc" strokeWidth={1.5} fill="url(#b1)" dot={false} />
              ...
              <div className="grid grid-cols-4 border-t border-[#e0e0e0] pt-3">
                {[['Advances', '1,856', '#1a7f4b'], ['Declines', '1,089', '#c0392b'], ['Unchanged', '136'], ['Breadth', '+767', '#1a7f4b']].map(([l, v, c]) => (
                  <div key={l} className="text-center"><div className="text-[16px] font-[600]" style={{ color: c ?? '#1d1d1f' }}>{v}</div><div className="text-[9px] text-[#7a7a7a] mt-0.5">{l}</div></div>
                ))}
              </div>
            </div>
            <div className="bg-[#f5f5f7] rounded-[18px] p-6 border border-[#e0e0e0]">
        ...
        <div className="max-w-[980px] mx-auto px-6 py-[80px] text-center">
          ...
          <div className="grid grid-cols-5 gap-4">
            {factors.map((f, i) => {
              ...
              return (
                <div key={f.name} className="bg-white border border-[#e0e0e0] rounded-[18px] p-5 text-left">
      ...
      <div className="bg-[#272729]">
        <div className="max-w-[980px] mx-auto px-6 py-[80px]">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="bg-[#2a2a2c] rounded-[18px] p-6 text-center border border-[#333]">
              <Users size={24} className="text-[#2997ff] mx-auto mb-3" />
              <p className="text-[14px] text-[#cccccc] mb-4">"StockStory changed how I research companies."</p>
            ...
            <div className="bg-[#2a2a2c] rounded-[18px] p-6 text-center border border-[#333]">
              ...
              <span className="inline-flex items-center gap-1 text-[11px] text-[#7a7a7a] bg-[#333] px-3 py-1 rounded-[9999px]"><Shield size={12} /> G2 Leader 2024-25</span>
            </div>
            <div className="bg-[#2a2a2c] rounded-[18px] p-6 text-center border border-[#333]">
              <BarChart3 size={24} className="text-[#2997ff] mx-auto mb-3" />
              <div className="grid grid-cols-1 gap-3">
                {[['2M+', 'Users'], ['10M+', 'Research Reports'], ['250M+', 'Data Points']].map(([v, l]) => (
                  <div key={l}><div className="text-[24px] font-[600] text-white">{v}</div><div className="text-[11px] text-[#7a7a7a]">{l}</div></div>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="max-w-[980px] mx-auto px-6 py-[80px]">
          <div className="grid grid-cols-5 gap-6 text-center">
            {([
              ...
              <div key={i}>
                <div className="w-10 h-10 rounded-full bg-white flex items-center justify-center mx-auto mb-3 border border-[#e0e0e0]">
