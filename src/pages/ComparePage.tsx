...
export default function ComparePage() {
  ...
  const loadCompany = useCallback(
    async (symbol: string, side: "left" | "right") => {
  ...
  const summaryText = useMemo(() => {
    ...
    for (const g of ["quality", "growth", "valuation", "risk", "momentum"]) {
      ...
      const winner =
        l > r
          ? (leftResult.companyName ?? leftSym)
          : (rightResult.companyName ?? rightSym);
      winners.push(`${winner} scores higher on ${g}`);
    }
    if (!winners.length)
      return `${leftResult.companyName ?? leftSym} and ${rightResult.companyName ?? rightSym} have broadly similar profiles.`;
    return winners.join(". ") + ".";
    ...
    if (error) {
      return (
        ...
        >
          ...
          <button
            onClick={() => loadCompany(symbol, side)}
            style={{
              fontSize: 12,
              color: S.action,
              background: "none",
    ...
    return (
      <PremiumCard padding="20px" style={{ flex: 1 }}>
          ...
          {result.prediction?.keyStrengths?.slice(0, 2).map((s, i) => (
            <span
              ...
              style={{
                ...
                background: "var(--ss-positive-soft)",
                color: S.positive,
                fontWeight: 600,
              }}
            >
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 16 }}>
          <button
            onClick={() => handleInvest(symbol, result.companyName)}
            style={{
              flex: 1,
              padding: "10px",
              fontSize: 13,
          ...
          <button
            onClick={() => handleTrack(symbol, result.companyName)}
            style={{
              flex: 1,
              padding: "10px",
              fontSize: 13,
    ...
    return (
      ...
      >
        ...
        <div style={{ flex: 1, display: "flex", alignItems: "center", gap: 8 }}>
          ...
          {winner === "left" && (
            <Check size={14} color={S.positive} style={{ flexShrink: 0 }} />
          )}
    ...
    <div style={{ minHeight: "100vh", background: S.bg }}>
      ...
      <div style={{ paddingTop: 16, paddingBottom: 40 }}>
        ...
        >
          ...
          {/* Company selectors */}
          <div
            style={{ display: "flex", gap: mobile ? 12 : 24, marginBottom: 24 }}
          >
            {/* Left slot */}
              ...
              {!rightSym ? (
                ...
                >
                  ...
                  <input
                    ...
                    value={rightInput}
                    onChange={(e) =>
                      setRightInput(e.target.value.toUpperCase())
                    }
          ...
          {bothLoaded && (
            <>
              ...
              <PremiumCard style={{ marginBottom: 24 }} padding="20px">
                ...
                {FACTOR_GROUPS.map((g) => (
                  <FactorRow
                    key={g.key}
                    label={g.label}
                    leftScore={getFactorScore(leftResult, g.key)}
                    rightScore={getFactorScore(rightResult, g.key)}
                  />
                ))}
              </PremiumCard>
              ...
              <PremiumCard padding="20px">
                ...
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                  <button
                    onClick={() => productNavigate("stock", leftSym)}
                    style={{
                      padding: "10px 20px",
                      fontSize: 13,
                      fontWeight: 600,
                  ...
                  >
                    <ArrowUpRight size={14} /> Research{" "}
                    {leftResult.companyName ?? leftSym}
                  </button>
                  <button
                    onClick={() => productNavigate("stock", rightSym)}
                    style={{
                      padding: "10px 20px",
                      fontSize: 13,
                      fontWeight: 600,
                  ...
                  {[leftSym, rightSym].map((sym, i) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() =>
                          handleTrack(
                            sym,
                            i === 0
                              ? leftResult.companyName
                              : rightResult.companyName,
                          )
                        }
                        style={{
                          padding: "10px 20px",
                          fontSize: 13,
                          fontWeight: 600,
                          color: tracked ? S.positive : S.ink2,
                          border: `1px solid ${tracked ? "var(--ss-positive)" : S.border}`,
                          borderRadius: S.radiusSm,
                          background: "none",
                          cursor: "pointer",
                          display: "flex",
                      ...
                      >
                        {tracked ? <Check size={14} /> : <Sparkles size={14} />}{" "}
                        {tracked ? "Tracked" : "Track"} {sym}
                      </button>
                    );
                  })}
                  <button
                    onClick={() => {
                      setInvestSym(leftSym);
                      setInvestName(leftResult.companyName ?? leftSym);
                      setShowInvest(true);
                    }}
                    style={{
                      padding: "10px 20px",
                  ...
                  <button
                    onClick={() => {
                      setInvestSym(rightSym);
                      setInvestName(rightResult.companyName ?? rightSym);
                      setShowInvest(true);
                    }}
                    style={{
                      padding: "10px 20px",
