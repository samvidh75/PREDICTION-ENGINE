...
import CalmMarketNewsStoryPanel from "../components/news/CalmMarketNewsStoryPanel";
...
import CompanySuperpage from "../components/company/CompanySuperpage";
...
import { addTickerToWatchlist, isTickerInWatchlist, removeTickerFromWatchlist, getWatchlists } from "../services/portfolio/watchlistStore";
...
export default function CompanyUniversePage(): JSX.Element {
  ...
  const onToggleWatchlist = () => {
    ...
    if (watchlistHasTicker) removeTickerFromWatchlist(listId, model.ticker);
    else addTickerToWatchlist(listId, model.ticker);
  ...
  const { setTier } = usePremiumEntitlement();

  useEffect(() => {
    if (!qaForcePremium) return;
    setTier("premium");
  }, [qaForcePremium, setTier]);

  const beginner = qaForcePremium ? false : experienceLevel === "beginner";
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      const el = document.getElementById("company-market-events-layer");
      el?.scrollIntoView({
        behavior: prefersReducedMotion ? "auto" : "smooth",
        block: "start",
      });
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#020304]">
        ...
        <div className="relative h-full">
          ...
          <div className={isMobile ? "relative w-full" : "absolute left-0 top-0 w-full sm:w-[600px]"}>
            ...
            <div className="mt-3 text-[11px] uppercase tracking-[0.18em] text-white/45">
              Workspace, clearly framed • calm context
            </div>
          </div>
              ...
              >
                ...
                <div className="mt-3 text-[13px] leading-[1.7] text-white/80">
                  ...
                  <div className="mt-5 rounded-[20px] border border-white/10 bg-black/20 p-4">
                    <div className="text-[10px] uppercase tracking-[0.18em] text-white/60">Strategic summary</div>
      ...
      {/* Company Superpage (progressive layered exploration; SEBI-safe educational framing) */}
      <div className="relative z-[12]">
        {/* Lazy hydration through ProgressiveDisclosure: content is isolated per layer */}
        <CompanySuperpage
          model={model}
          confidenceState={confidenceState}
      ...
      <section ref={chartsSectionRef} className="relative z-[12] px-6 sm:px-[72px] pb-14">
        <div className="mx-auto max-w-[1680px]">
            ...
            <div>
              <div className="text-[12px] uppercase tracking-[0.18em] text-white/70">Business Intelligence Visuals</div>
              <div className="mt-3 text-[22px] font-medium text-white/92">Story-first market learning</div>
              <div className="mt-3 text-[14px] leading-[1.9] text-white/75 max-w-[90ch]">
                These visuals connect business evolution to current market conditions, clearly.
              </div>
            </div>

            <div className="text-[11px] uppercase tracking-[0.18em] text-white/45">
              mode: {isMobile ? "mobile-calm" : "calm"} • calm overlays
            </div>
          </div>
      ...
      {/* Calm, connected company intelligence (SEBI-safe; progressive story stack) */}
      <CalmMarketNewsStoryPanel
        ...
        company={model}
        isMobile={isMobile}
        layerOrder={["company", "earnings", "educational"]}
      />
      ...
      <div className="mt-6 text-[12px] uppercase tracking-[0.18em] text-white/45 px-6 sm:px-[72px] pb-10">
        Context-first intelligence • calm pacing
      </div>
    </div>
  );
}
