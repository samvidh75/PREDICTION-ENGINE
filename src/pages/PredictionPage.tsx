...
function momentumColor(value: number | null): string {
  ...
  return value >= 50 ? colors.success : value >= 35 ? colors.danger : colors.danger;
}

function ScoreGauge({ score }: { score: number }) {
  const color = score >= 80 ? colors.success : score >= 65 ? colors.success : score >= 50 ? colors.danger : score >= 35 ? colors.danger : colors.danger;
  const offset = 138 - (138 * score) / 100;
  return (
    <svg width="56" height="56" viewBox="0 0 56 56" style={{ flexShrink: 0 }}>
      <circle cx="28" cy="28" r="22" fill="none" stroke={colors.hairline} strokeWidth="4" />
      ...
      <text x="28" y="30" textAnchor="middle" fill={colors.ink} fontSize="11" fontWeight="bold">{score}</text>
    </svg>
  );
}
...
function FactorBar({ label, value, max }: { label: string; value: unknown; max?: number }) {
  ...
  return (
    <div style={{ marginBottom: "4px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "11px", color: colors.body }}>
        <span>{label}</span>
        <span>{displayVal}</span>
      </div>
      <div style={{ height: "4px", background: colors.hairline, borderRadius: "2px", overflow: "hidden" }}>
        <div style={{ width: `${pct}%`, height: "100%", background: positive ? colors.success : colors.danger, borderRadius: "2px", transition: "width 0.3s" }} />
      </div>
    </div>
  );
}
...
function PredictionCard({ ticker, prediction, loading, failed, onRefresh }: { ticker: string; prediction: Prediction | null; loading: boolean; failed: boolean; onRefresh: (t: string) => void }) {
  if (!prediction) {
    return (
      <div style={{ background: colors.surface, border: `1px solid ${colors.hairline}`, borderRadius: "12px", padding: "20px" }}>
        {failed && !loading ? (
          <div style={{ textAlign: "center", padding: "20px" }}>
            <p style={{ color: colors.body, fontSize: "13px", margin: "0 0 12px" }}>
              {"Couldn't load a prediction for "}{ticker}.
            </p>
            <button
              onClick={() => onRefresh(ticker)}
              style={{ padding: "6px 14px", fontSize: "12px", border: `1px solid ${colors.hairlineStrong}`, borderRadius: "6px", background: "transparent", color: colors.primary, cursor: "pointer" }}
            >
              Retry
            </button>
          </div>
        ) : (
          <div style={{ textAlign: "center", padding: "20px", color: colors.mute, fontSize: "13px" }}>
            Searching {ticker}...
          </div>
        )}
      </div>
  ...
  const color = CLASS_COLORS[prediction.classification] || colors.mute;
  return (
    <div style={{ background: colors.surface, border: `1px solid ${colors.hairline}`, borderRadius: "12px", padding: "20px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px" }}>
        <div>
          <h3 style={{ margin: 0, fontSize: "16px", fontWeight: 600, color: colors.ink }}>{ticker}</h3>
          {prediction.sector && <span style={{ fontSize: "11px", color: colors.mute }}>{prediction.sector}</span>}
        </div>
      ...
      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "12px" }}>
        ...
        <span style={{ padding: "3px 8px", borderRadius: "4px", fontSize: "11px", background: colors.surfaceElevated, color: colors.body }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
        <div>
          <p style={{ fontSize: "11px", color: colors.mute, margin: "0 0 6px", fontWeight: 600 }}>Technical</p>
        ...
        <div>
          <p style={{ fontSize: "11px", color: colors.mute, margin: "0 0 6px", fontWeight: 600 }}>Fundamental</p>
      ...
      {prediction.momentumBreakdown && (
        <div style={{ marginTop: "16px", padding: "12px", background: colors.surface, borderRadius: "8px", border: `1px solid ${colors.hairline}` }}>
          <p style={{ fontSize: "11px", color: colors.mute, margin: "0 0 8px", fontWeight: 600 }}>Momentum Breakdown (from real EODHD data)</p>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: "8px", textAlign: "center" }}>
            <div>
              <div style={{ fontSize: "10px", color: colors.body }}>Short Term</div>
            ...
            <div>
              <div style={{ fontSize: "10px", color: colors.body }}>Medium Term</div>
            ...
            <div>
              <div style={{ fontSize: "10px", color: colors.body }}>Trend</div>
            ...
            <div>
              <div style={{ fontSize: "10px", color: colors.body }}>Overall</div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
        <div>
          <p style={{ fontSize: "11px", color: colors.mute, margin: "0 0 6px", fontWeight: 600 }}>Valuation</p>
        ...
        <div>
          <p style={{ fontSize: "11px", color: colors.mute, margin: "0 0 6px", fontWeight: 600 }}>Risk</p>
      ...
      <button
        onClick={() => onRefresh(ticker)}
        style={{ marginTop: "12px", padding: "6px 12px", fontSize: "11px", border: `1px solid ${colors.hairlineStrong}`, borderRadius: "6px", background: "transparent", color: colors.body, cursor: "pointer", width: "100%" }}
      >
        Refresh Prediction
      </button>
    </div>
  ...
  return (
    <div style={{ marginBottom: "24px" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "12px" }}>
        ...
        <button onClick={onRefresh} style={{ padding: "4px 10px", fontSize: "11px", border: `1px solid ${colors.hairlineStrong}`, borderRadius: "4px", background: "transparent", color: colors.mute, cursor: "pointer" }}>
          Refresh
        </button>
      </div>
      <div style={{ display: "flex", gap: "10px", overflowX: "auto", paddingBottom: "4px" }}>
        {prices.map((p) => {
          ...
          const arrow = chg >= 0 ? "▲" : "▼";
          return (
            <div key={p.ticker} style={{ background: colors.surface, border: `1px solid ${colors.hairline}`, borderRadius: "8px", padding: "10px 14px", flexShrink: 0, minWidth: "120px" }}>
              ...
              <div style={{ fontSize: "11px", color: chg >= 0 ? colors.success : colors.danger }}>{arrow} {chg.toFixed(2)}%</div>
              <div style={{ fontSize: "9px", color: colors.mute, marginTop: "2px" }}>by {p.user_id.slice(0, 8)}</div>
            </div>
          );
        })}
      </div>
...
function SubmissionsPanel({ userId }: { userId: string }) {
  ...
  return (
    <div style={{ marginTop: "24px", background: colors.surface, border: `1px solid ${colors.hairline}`, borderRadius: "12px", padding: "20px" }}>
      <h2 style={{ margin: "0 0 12px", fontSize: "14px", fontWeight: 600 }}>Submit Price (User-as-Data-Source)</h2>
      <p style={{ fontSize: "11px", color: colors.mute, marginBottom: "14px" }}>
      ...
      <button onClick={handleSubmit} disabled={status === "submitting"} style={{
        ...
        background: status === "done" ? colors.success : colors.primary,
        color: colors.onPrimary, cursor: "pointer", width: "100%",
      }}>
        {status === "submitting" ? "Submitting..." : status === "done" ? "✓ Submitted" : "Submit Price"}
      </button>
      {msg && <p style={{ fontSize: "11px", color: status === "done" ? colors.success : colors.danger, margin: "6px 0 0" }}>{msg}</p>}
    </div>
  );
}

const inputStyle: React.CSSProperties = {
  padding: "8px 10px", fontSize: "12px", border: `1px solid ${colors.hairlineStrong}`, borderRadius: "6px",
  background: colors.surfaceElevated, color: colors.ink, outline: "none",
};
...
export default function PredictionPage() {
  ...
  return (
    <div style={{ maxWidth: "1200px", margin: "0 auto", padding: "28px 16px 24px" }}>
      <div style={{ marginBottom: "24px" }}>
        ...
        <h1 style={{ fontFamily: typography.serifFamily, fontStyle: "italic", fontWeight: 500, fontSize: 28, margin: "6px 0 6px", color: colors.ink, letterSpacing: "-0.01em" }}>
          Fifty factors. One verdict.
        </h1>
        <p style={{ fontSize: "13.5px", color: colors.body, margin: 0 }}>
          User-sourced live prices, real filings, no shortcuts — technical, fundamental,
          valuation, and risk factors rolled into a single composite score.
        </p>
      </div>
      ...
      <div style={{ display: "flex", gap: "8px", marginBottom: "20px" }}>
        <input
          ...
          style={{
            flex: 1, padding: "10px 14px", fontSize: "13px", border: `1px solid ${colors.hairlineStrong}`,
            borderRadius: "8px", background: colors.surfaceElevated, color: colors.ink, outline: "none",
          }}
        />
        <button onClick={addTicker} style={{
          padding: "10px 20px", fontSize: "13px", border: "none", borderRadius: "8px",
          background: colors.accentRedSoft, color: colors.primary, cursor: "pointer", fontWeight: 600,
        }}>
          Analyze
        </button>
      </div>
      ...
      <div style={{ display: "flex", gap: "8px", flexWrap: "wrap", marginBottom: "20px" }}>
        {PSE_30.slice(0, 12).map((t) => (
          <button
            ...
            style={{
              padding: "4px 10px", fontSize: "10px", border: `1px solid ${colors.hairline}`, borderRadius: "4px",
              background: tickers.includes(t) ? colors.accentRedSoft : "transparent",
              color: tickers.includes(t) ? colors.primary : colors.mute, cursor: "pointer",
            }}
          >
            {t}
          </button>
      ...
      <div style={{ marginTop: "32px", padding: "16px", borderRadius: "8px", background: colors.surface, border: `1px solid ${colors.hairline}` }}>
        <h3 style={{ fontSize: "12px", fontWeight: 600, margin: "0 0 8px", color: colors.mute }}>About the Prediction Engine</h3>
        <div style={{ fontSize: "11px", color: colors.mute, lineHeight: 1.6 }}>
