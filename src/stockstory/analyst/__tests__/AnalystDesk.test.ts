...
describe('AnalystTaskRegistry', () => {
  ...
  it('creates valid task', () => {
    const r = registry.validateCreate({ taskType: 'company_deep_dive', symbol: 'BDO' });
    expect(r.valid).toBe(true);
  });

  it('rejects invalid task type', () => {
    const r = registry.validateCreate({ taskType: 'invalid', symbol: 'BDO' });
...
describe('AnalystTaskRunner', () => {
  ...
  it('runs task to completion', async () => {
    const task = await runner.createAndRun({ taskType: 'company_deep_dive', symbol: 'BDO' });
