...
export class ResearchNarrativeService {
  generateCompanyThesis(input: LensoryResearchInput): string {
  ...
  generateBullCase(input: LensoryResearchInput): string {
  ...
  generateBearCase(input: LensoryResearchInput): string {
  ...
  generateWhatChanged(input: LensoryResearchInput): string {
  ...
  generateWhyItMatters(input: LensoryResearchInput): string {
  ...
  generateWatchNext(input: LensoryResearchInput): string {
  ...
  generateRiskSummary(input: LensoryResearchInput): string {
  ...
  generatePeerContext(input: LensoryResearchInput): string {
    const { peerContext, sector, score } = input;

    if (!peerContext) {
      return `Within the ${sector} sector, this company scores ${score}/100 on the Lensory research framework. Sector context is available when more peer data is loaded.`;
    }

    const parts: string[] = [
      `Among ${peerContext.peerCount} companies in the ${peerContext.sector} sector tracked by Lensory, ${input.companyName} ranks at the ${peerContext.percentileRank}th percentile.`,
    ];
  ...
  generateComplianceSafeLabel(input: LensoryResearchInput): string {
  ...
  generateFullNarrative(input: LensoryResearchInput): LensoryNarrativeOutput {
    return {
      ...
      methodologyNote: 'This research summary is generated by Lensory\'s deterministic analytical engine. Scores are derived from financial fundamentals, technical indicators, and sector-relative percentile rankings.',
      complianceSafeLabel: this.generateComplianceSafeLabel(input),
    };
  }

  private generateConfidenceNote(input: LensoryResearchInput): string {
