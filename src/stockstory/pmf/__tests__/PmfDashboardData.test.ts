...
describe('PmfDashboardData', () => {
  ...
  it('generates dashboard payload from events', async () => {
    ...
    const testEvents = [
      ...
      { eventType: 'discovery', action: 'search', userId: 'user_1', metadata: { query: 'SM', label: 'Search' } },
      { eventType: 'engagement', action: 'page_view', userId: 'user_1', metadata: { page: '/stock/SM', label: 'Stock view', symbol: 'SM' } },
      { eventType: 'feedback', action: 'rating', userId: 'user_1', metadata: { symbol: 'SM', component: 'thesis', rating: '5', label: 'Rating 5' } },
    ];
