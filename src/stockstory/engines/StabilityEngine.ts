...
export class StabilityEngine {
  evaluate(inputs: EngineInputs): StabilityEngineOutput {
    ...
    const method = anyPercentile ? ` (sector-percentile vs ${profile.name} peers)` : '';
    const commentary =
      compositeScore >= 80 ? `Strong balance sheet stability${method}: low leverage and solid liquidity coverage.`
      : compositeScore >= 60 ? `Above-average financial stability${method}. Leverage and coverage metrics are healthy.`
      : compositeScore >= 40 ? `Average stability profile${method}. Leverage and liquidity are in line with sector norms.`
      : `Below-average stability${method}. Elevated leverage or weak coverage metrics warrant attention.`;
