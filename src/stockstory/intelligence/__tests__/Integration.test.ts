...
describe('KnowledgeBase', () => {
  ...
  it('stores and retrieves documents', () => {
    kb.ingest({
      id: '1',
      content: 'BDO has strong operating margins above 25%',
      source: 'fundamentals',
      symbol: 'BDO',
      tags: ['margins', 'it'],
    ...
    const results = kb.query('BDO margins', 5);
  ...
  it('finds documents by symbol', () => {
    kb.ingestMany([
      { id: '1', content: 'BDO info', source: 'test', symbol: 'BDO', tags: [], timestamp: '2025-01-15' },
      { id: '2', content: 'ICT info', source: 'test', symbol: 'ICT', tags: [], timestamp: '2025-01-15' },
    ]);
    expect(kb.findBySymbol('BDO')).toHaveLength(1);
    expect(kb.findBySymbol('ICT')).toHaveLength(1);
  });
