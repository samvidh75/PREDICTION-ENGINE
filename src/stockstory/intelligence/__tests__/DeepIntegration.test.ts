 ...
 * Uses the same realistic SM-like test data from Integration.test.ts.
 */
...
describe('Individual Engine Unit Tests', () => {
  ...
  it('MemoryEngine records and queries', () => {
    ...
    memoryEngine.recordEngineRun('SM', 'MoatEngine', { score: 65 });
    memoryEngine.recordSignal('SM', 'sig_wide_moat', { strength: 'strong' });
    const results = memoryEngine.getForSymbol('SM');
    expect(results.length).toBe(2);

    const report = memoryEngine.buildReport('SM');
