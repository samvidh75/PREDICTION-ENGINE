...
export class TechnicalSnapshotRefresh implements IngestionJob {
  ...
  protected async persistSnapshot(_symbol: string, _data: TechnicalSnapshotFields): Promise<void> {
    // NOTE: Actual DB persistence handled by calling script — see
    // PersistedTechnicalSnapshotRefresh in scripts/refresh-technical-snapshots.ts
    // for a real implementation writing to the technical_snapshots table.
  }
}
