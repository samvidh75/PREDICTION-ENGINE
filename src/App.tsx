...
type PublicRoute = "home" | "scanner" | "stock" | "watchlist" | "compare" | "login" | "pricing" | "search" | "forgot-password";
...
function readRoute(): { page: PublicRoute; symbol: string } {
  ...
  if (segments[0] === "stock" && segments[1]) {
    return { page: "stock", symbol: segments[1].toUpperCase().trim() };
  }
  if (segments[0] === "login") return { page: "login", symbol: "" };
  if (segments[0] === "register") return { page: "login", symbol: "" };
  if (segments[0] === "forgot-password") return { page: "forgot-password", symbol: "" };
  if (segments[0] === "pricing") return { page: "pricing", symbol: "" };
  if (segments[0] === "scanner") return { page: "scanner", symbol: "" };
  if (segments[0] === "compare") return { page: "compare", symbol: "" };
  if (segments[0] === "watchlist") return { page: "watchlist", symbol: "" };
  if (segments[0] === "search") return { page: "search", symbol: "" };

  const requested = url.searchParams.get("page") || "home";
  const page: PublicRoute = ["home","scanner","stock","watchlist","compare","login","forgot-password","pricing","search"].includes(requested)
    ? requested as PublicRoute : "home";
  return {
    page,
    symbol: (url.searchParams.get("id") || url.searchParams.get("symbol") || "").toUpperCase().trim(),
  };
}

const ROUTE_META: Record<string, { title: string; desc: string }> = {
  home:     { title: "StockStory India — AI-Powered Stock Research & Analysis Platform", desc: "AI-powered research on Indian stocks. Proprietary scores across 5 dimensions: Quality, Growth, Valuation, Momentum & Risk." },
...
function PublicRouter() {
  ...
  if (route.page === "stock") return <StockPage symbol={route.symbol} />;
  ...
  if (route.page === "forgot-password") return <ForgotPasswordPage />;
