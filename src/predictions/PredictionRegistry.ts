...
export class PredictionRegistry {
  ...
  async createPrediction(input: CreatePredictionInput): Promise<PredictionRecord> {
    ...
    // Deliberately no RETURNING clause: insert, then read the row back by
    // id. Functionally equivalent and avoids a real datatype-mismatch bug
    // in the local SQLite dev fallback's INSERT ... RETURNING handling on
    // this table shape (verified: the identical INSERT with RETURNING
    // stripped succeeds; Postgres production isn't affected either way).
    await pool.query(
    ...
    const result = await pool.query(`SELECT * FROM prediction_registry WHERE id = $1`, [id]);
  ...
  async createPredictionsBatch(inputs: CreatePredictionInput[]): Promise<PredictionRecord[]> {
    ...
    try {
      ...
      for (const input of inputs) {
        ...
        const result = await client.query(`SELECT * FROM prediction_registry WHERE id = $1`, [id]);
