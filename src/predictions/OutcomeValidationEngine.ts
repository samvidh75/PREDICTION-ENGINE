...
export class OutcomeValidationEngine {
   ...
   *   2. Get the current PSEi benchmark level
  ...
  async validateAtHorizon(
    ...
    // 4. Fetch current benchmark level (PSEi) closest to referenceDate
  ...
  /**
   * Fetch the current PSEi benchmark level.
  ...
  private async fetchCurrentBenchmark(
    ...
    if (benchResult.rows.length > 0 && benchResult.rows[0].psei !== null) {
      return Number(benchResult.rows[0].psei);
    }

    // Fallback: daily_prices for the PSEI ticker itself
    const fallbackResult = await pool.query(
      `SELECT dp.close
       FROM daily_prices dp
       WHERE dp.symbol = 'PSEI'
         AND dp.trade_date <= $1
