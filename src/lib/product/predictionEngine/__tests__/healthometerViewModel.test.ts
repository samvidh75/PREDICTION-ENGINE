...
describe('healthometerViewModel', () => {
  it('builds Complete state when all dimensions have scores', () => {
    const vm = buildHealthometerViewModel(80, 60, 70, 65, 30, 55);
    expect(vm.overallStatus).toBe('Complete');
    expect(vm.overallScore).toBe(60);
    expect(vm.dimensions).toHaveLength(6);
  });

  it('shows Partial research context when some dims are missing', () => {
    const vm = buildHealthometerViewModel(80, null, 70, null, null, null);
  ...
  it('shows Not enough information when no dims available', () => {
    const vm = buildHealthometerViewModel(null, null, null, null, null, null);
  ...
  it('never renders N/A or NaN', () => {
    const vm = buildHealthometerViewModel(null, NaN, Infinity, undefined, 30, null);
