...
describe('computeResearchScore', () => {
  ...
  it('returns valid research stance for all stances list', () => {
    const allValidStances = /Very Healthy|Healthy|Unhealthy|Very Unhealthy|Not enough information/;
  ...
  it('high score + low confidence does not become Very Healthy', () => {
    ...
    expect(stance.stance).not.toBe('Very Healthy');
  });

  it('medium score + high risk maps to Unhealthy', () => {
    const stance = computeResearchScore({
      pe: 15, roe: 12, operating_margin: 0.15,
      revenue_growth: 0.05, debt_equity: 1.2, current_ratio: 1.2,
    }, 65).stance;
    expect(stance.stance).toBe('Unhealthy');
  });

  it('low score + high confidence maps to Unhealthy', () => {
    const stance = computeResearchScore({
