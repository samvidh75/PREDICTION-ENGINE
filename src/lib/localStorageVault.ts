...
export class LocalStorageVault {
  ...
  private searchHistoryCache: SearchHistoryEntry[] | null = null;
  ...
  async addSearchHistory(symbol: string, timestamp: number = Date.now()): Promise<void> {
    ...
    this.persistSearchHistory(history);
  }
  ...
  private getSearchHistoryRaw(): SearchHistoryEntry[] {
    if (this.searchHistoryCache) return this.searchHistoryCache;
    const raw = localStorage.getItem(`${this.KEY_PREFIX}search_history`);
    const history: SearchHistoryEntry[] = raw ? JSON.parse(raw) : [];
    this.searchHistoryCache = history;
    return history;
  }

  private persistSearchHistory(history: SearchHistoryEntry[]): void {
    this.searchHistoryCache = history;
