    ...
    configureServer(server) {
      server.middlewares.use(async (req, res, next) => {
        ...
        try {
          ...
          // ── Market Pulse — delegates to the real api/market-pulse.ts
          // handler (live PSEi-30 gainers/losers/index proxy from phisix). ──
          if (url.startsWith("/api/market-pulse")) {
            await callVercelHandler(server, path.resolve(root, "api/market-pulse.ts"), req, res);
            return;
          }
          ...
          if (url.startsWith("/api/market-universe")) {
            await callVercelHandler(server, path.resolve(root, "api/market-universe.ts"), req, res);
            return;
          }

          // ── Foreign Flow — delegates to the real api/foreign-flow/index.ts
          // handler (real net foreign buying/selling from the PSE Daily
          // Quotation Report, data/pse-foreign-flow.json). ──
          if (url.startsWith("/api/foreign-flow")) {
            await callVercelHandler(server, path.resolve(root, "api/foreign-flow/index.ts"), req, res);
            return;
          }

          // ── Market Data Quote — delegates to api/market-data/quote/[symbol].ts
          // (lightweight flat-shape single quote, used by PortfolioPage's
          // per-holding price refresh; distinct from the heavier api/stock). ──
          if (url.startsWith("/api/market-data/quote")) {
            const parsed = new URL(url, "http://localhost");
            const symbol = (parsed.searchParams.get("symbol") ?? decodeURIComponent(url.replace("/api/market-data/quote/", "").split("?")[0] ?? "")).toUpperCase();
            await callVercelHandler(server, path.resolve(root, "api/market-data/quote/[symbol].ts"), req, res, { symbol });
            return;
          }

          // ── Fundamentals — delegates to api/fundamentals/[symbol].ts,
          // serving the PSE Edge scrape output (data/pse-fundamentals.json). ──
          if (url.startsWith("/api/fundamentals")) {
            const parsed = new URL(url, "http://localhost");
            const symbol = (parsed.searchParams.get("symbol") ?? decodeURIComponent(url.replace("/api/fundamentals/", "").split("?")[0] ?? "")).toUpperCase();
            await callVercelHandler(server, path.resolve(root, "api/fundamentals/[symbol].ts"), req, res, { symbol });
            return;
          }
          ...
          // delegates to the real api/relative-strength.ts handler (real
          // EODHD daily OHLCV momentum over the PSEi-30, cached 6h). ──
          if (url.startsWith("/api/relative-strength")) {
            await callVercelHandler(server, path.resolve(root, "api/relative-strength.ts"), req, res);
            return;
          }

          // ── Backtest — delegates to the real api/backtest.ts handler
          // (real EODHD daily bars, SMA-50 crossover strategy, real
          // computeMetrics() output; no fabricated numbers). ──
          if (url.startsWith("/api/backtest")) {
            await callVercelHandler(server, path.resolve(root, "api/backtest.ts"), req, res);
            return;
          }
          ...
          if (url.startsWith("/api/intelligence")) {
            if (url.startsWith("/api/intelligence/stock")) {
              ...
              const inputs = {
                ...
                events: { events: [], lastUpdated: new Date(), fiscalYear: 2025, currency: 'PHP' },
          ...
          // ── Watchlist Intelligence — delegates to the real
          // WatchlistIntelligenceEngine (real persisted PSE data or honest
          // null, see stockResearchSnapshot.ts; no thesis-history/alert
          // persistence side effects in this dev shim, unlike the full
          // Fastify route in personalResearchRoutes.ts). ──
          if (url.startsWith("/api/watchlist-intelligence")) {
            const parsed = new URL(url, "http://localhost");
            const tickers = (parsed.searchParams.get("tickers") ?? "")
              .split(",").map((s) => s.trim().toUpperCase()).filter(Boolean);
            if (!tickers.length) {
              res.statusCode = 400;
              res.end(JSON.stringify({ error: "tickers required (comma-separated)" }));
              return;
            }
            const { WatchlistIntelligenceEngine } = (await server.ssrLoadModule(
              path.resolve(root, "src/services/personalization/WatchlistIntelligenceEngine.ts"),
            )) as typeof import("./src/services/personalization/WatchlistIntelligenceEngine");
            const changesOnly = parsed.searchParams.get("changesOnly");
            const intelligence = await WatchlistIntelligenceEngine.buildIntelligence(
              tickers, new Map(), { changesOnly: changesOnly === "true" || changesOnly === "1" },
            );
            res.setHeader("Content-Type", "application/json");
          ...
          // ── Analyst Workspace ─────────────────────────────────
          if (url.match(/^\/api\/analyst\/company\/[^/]+\/deep-dive/) && req.method === "GET") {
            const parts = url.split("?")[0].split("/");
            const symbol = decodeURIComponent(parts[4] ?? "").toUpperCase();
            if (!symbol) { res.statusCode = 400; res.end(JSON.stringify({ error: "Valid symbol required" })); return; }
            const { analystDeskService } = (await server.ssrLoadModule(
              path.resolve(root, "src/stockstory/analyst/AnalystDeskService.ts"),
            )) as typeof import("./src/stockstory/analyst/AnalystDeskService");
            res.setHeader("Content-Type", "application/json");
            try {
              const result = await analystDeskService.getDeepDive(symbol);
              res.end(JSON.stringify({ ok: true, symbol, data: result.publicOutput }));
            } catch {
              res.end(JSON.stringify({ ok: false, symbol, data: { summary: "Research deep dive unavailable.", limitations: ["Limited information"] } }));
            }
            return;
          }

          if (url.startsWith("/api/analyst/qa") && req.method === "POST") {
            let body = "";
            ...
            req.on("end", async () => {
              try {
                ...
                res.setHeader("Content-Type", "application/json");
                res.end(JSON.stringify({ ok: true, data: result.publicOutput }));
              } catch (e: any) {
                res.statusCode = 500;
