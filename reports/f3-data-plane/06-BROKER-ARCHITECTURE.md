...
| `InMemoryProviderBrokerStore.ts` | Deterministic single-instance store for tests and explicitly allowed development mode. |
| `RedisProviderBrokerStore.ts` | Redis contract boundary for namespaced keys, TTL-backed counters/cooldowns/negative cache, and distributed locks. |
| `ProviderRequestBroker.ts` | Cache lookup, stale-while-revalidate, single-flight coalescing, quota checks, retries, cooldown handling, negative cache, and ledger recording. |
