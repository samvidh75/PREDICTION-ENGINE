...
node -e "const { ScreenerParser } = require('./src/services/providers/parsers/ScreenerParser'); const p = new ScreenerParser(); fetch('https://www.phisix-api3.appspot.com/company/RELIANCE/').then(r => r.text()).then(h => console.log(JSON.stringify(p.parseRatiosPage(h), null, 2)))"
