...
// of which are null today because there is no free fundamentals feed for
// PSE (see the EODHD_KEY comment further down). This is not a "350+
// parameter" model; that claim was inaccurate and has been removed.
...
// own live ticker data. It has only the latest price/change, no history or
// fundamentals; 52-week range and the price chart come from EODHD instead
// (see fetchEodhdHistory above).
...
export default async function handler(
  ...
  try {
    const [priceData, eodhdBars] = await Promise.all([
      fetchPSEPrice(symbol),
    ...
    // Fetch real fundamentals from Yahoo Finance PSE + sector estimates
    ...
    const healthScores = calculatePSEHealthScore({
      symbol,
      ...
      profitGrowth: fundamentalsData.profitGrowth,
      marketCap: fundamentalsData.marketCap ?? priceData.marketCap ?? null,
      high52w: fiftyTwoWeek?.high ?? null,
      low52w: fiftyTwoWeek?.low ?? null,
      beta: fundamentalsData.beta,
      rsi: null,
      macd: null,
      volume: fundamentalsData.volume,
    ...
    const response = {
      ...
      volatility: null,
      interestCoverage: fundamentalsData.interestCoverage,
      industryPe: fundamentalsData.industryPe,
      revenueGrowth: fundamentalsData.revenueGrowth,
      profitGrowth: fundamentalsData.profitGrowth,
      above50Dma: null,
      scores: {
      ...
      // already renders a graceful "No chart data available" state) when
      // the free-tier fetch fails or the symbol has no EODHD coverage.
      priceChart,
      source: eodhdBars ? 'phisix+eodhd' : 'phisix',
