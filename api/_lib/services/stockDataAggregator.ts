...
import { CompleteStockData } from "../data/universe.js";
...
class StockDataAggregator {
  ...
  private dataSources: DataSource[] = [
    ...
    },
    {
      name: "bseindia",
      priority: 2,
      fetch: this.fetchFromPSE.bind(this),
    },
    {
      name: "nseindia",
      priority: 2,
  ...
  private async fetchFromYahoo(symbol: string): Promise<Partial<CompleteStockData>> {
    try {
      const yahooSymbol = symbol.includes(".") ? symbol : `${symbol}`;  // No .NS suffix for PSE
  ...
  /**
   * Fetch from PSE Philippines — phisix API
   */
  private async fetchFromPSE(symbol: string): Promise<Partial<CompleteStockData>> {
    try {
      const url = `https://phisix-api3.appspot.com/stocks/${symbol.toLowerCase()}.json`;
      ...
      return {
        ...
        price: {
          current: stock.price?.amount || 0,
          change: stock.percentChange || 0,
          volume: stock.volume || 0,
        },
        fundamentals: {
          pe: 0, // phisix doesn't provide fundamentals — PSE EDGE needed
          pb: 0,
        },
      } as unknown as Partial<CompleteStockData>;
    } catch (error) {
      console.error(`PSE phisix fetch failed for ${symbol}:`, error);
      return {};
    }
  }
