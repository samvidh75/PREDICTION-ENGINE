...
await test('sql.js WASM load and in-memory CRUD', async () => {
  ...
  db.run(
    'CREATE TABLE portable_test (id INTEGER PRIMARY KEY, value TEXT)',
  );

  db.run(
    'INSERT INTO portable_test (value) VALUES (?)',
    ['verified'],
  );
  ...
  if (value !== 'verified') {
    throw new Error(
      `Expected verified, received ${String(value)}`,
    );
  }
