/**
 * audit-rc3.ts — RC3 Release Gate.
 * Single command aggregating all critical checks.
 *
 * Usage:
 *   npx tsx scripts/audit-rc3.ts                     # localhost:3000
 *   npx tsx scripts/audit-rc3.ts --production         # production URLs
 *   API_BASE_URL=https://example.com npx tsx scripts/audit-rc3.ts
 */
...
const CHECKS: AuditCheck[] = [
  ...
  { name: "static_assets", command: `API_BASE_URL=${API_BASE} npx tsx scripts/smoke-static-assets.ts`, critical: false },
];
...
function main(): void {
  ...
  if (criticalFailed > 0) {
    console.log("\nRC3 gate: FAIL (critical checks failed)");
    process.exit(1);
  }

  if (failed > 0) {
    console.log("\nRC3 gate: PASS (non-critical warnings only)");
    process.exit(0);
  }
