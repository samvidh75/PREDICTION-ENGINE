...
const OUT = path.resolve(__dirname, "../.tmp/part-bf-after");
...
async function main() {
  ...
  const serverProcess = spawn("npx", ["vite", "preview", "--port", "4176", "--strictPort"], {
  ...
  for (const vp of VIEWPORTS) {
    ...
    for (const route of ROUTES) {
      let url = `http://localhost:4176/?page=${route.page}`;
    ...
    if (vp.width <= 768) {
      try {
        await page.goto(`http://localhost:4176/?page=invest&id=RELIANCE`, { waitUntil: "domcontentloaded", timeout: 20000 });
        await page.waitForTimeout(800);
      ...
      try {
        await page.goto(`http://localhost:4176/?page=dashboard`, { waitUntil: "domcontentloaded", timeout: 20000 });
