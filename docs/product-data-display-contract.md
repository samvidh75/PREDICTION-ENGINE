...
- User-entered state (notes, portfolio positions, watchlists) stored separately from computed state (scores, signals, factors)
- Provider-derived state (scores, rankings) is never stored in user workspace — only computed from current data
- Display-ready state combines user and computed data at render time
- No fake persistence — localStorage is labeled "Saved on this device" unless cloud sync is verified
- No fake cloud sync or cross-device persistence without real backend support

## Persistence Language Rules

- Local-only storage: "Saved on this device"
- Cloud-backed storage: "Saved to your account"
- Local + cloud: "Saved locally and synced to your account"
- No "synced" or "backed up" unless persistence is verified
- No "cloud" or "server" references for local-only data

## Thesis Snapshot Rules

- Snapshots record point-in-time research signal state
- First snapshot generates "Tracking begins now" event
- Changed score generates "signal_changed" event
- Risk dropping below 40 generates "risk_rising" event
- Factor changes without score change generate "factor_changed" event
- No prior snapshot vs current comparison is always valid
- Snapshots never store raw provider payloads
- No fake historical snapshots

## Change Event Rules

- Events generated by comparing current snapshot against prior snapshot
- No prior snapshot → "Tracking begins now" (not fake alert history)
- Real score change → "Research score changed from X to Y"
- Real risk change → "Risk indicators changed — review before proceeding"
- No fake delivery claims (no "email sent", "notification pushed" unless real)

## Freshness Wording Rules

- Recent (<24h): "Updated today"
- 24-48h: "Updated yesterday"
- 3-7 days: "Updated N days ago"
- 7-30 days: "Needs review"
- >30 days or missing: "Awaiting latest research cycle"
- No "data unavailable", "provider failed", "stale data", "backend timeout"

## Portfolio Calculation Rules

- Portfolio value only computed when user provides shares AND real quote exists
- No fake market value when shares or quote missing
- Movement only computed when both average price AND quote exist
- CSV import validates each row independently; malformed rows rejected with friendly copy
- No fake P&L, no fake broker sync, no fake broker connection

## No Fake Sync or Delivery

- No fake broker sync indicators
- No fake alert delivery (email/push) without real service
- No fake portfolio import success for malformed data
- No fake watchlist sync across devices without backend support
- No fake cross-device persistence
...
- Workspace tests assert snapshot creation, change detection, freshness labels
- Portfolio tests assert no fake P&L when data missing
